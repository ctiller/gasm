...
/- REF: docs/TARGETS/LINUX.md#31-elf64-layout-header-structure -/
/-- Serializes a standard 64-bit ELF file header (Elf64_Ehdr) into 64 bytes. -/
def serializeElf64Header (h : Elf64_Ehdr) : ByteArray :=
  Gasm.Targets.ELF.serializeElf64Header h

/- REF: docs/TARGETS/LINUX.md#31-elf64-layout-header-structure -/
/-- Serializes a 64-bit ELF program header entry (Elf64_Phdr) into 56 bytes. -/
def serializeProgramHeader (p : Elf64_Phdr) : ByteArray :=
  Gasm.Targets.ELF.serializeElf64Phdr p

/- REF: docs/TARGETS/LINUX.md#31-elf64-layout-header-structure -/
/-- Serializes a 64-bit ELF section header entry (Elf64_Shdr) into 64 bytes. -/
def serializeSectionHeader (s : Elf64_Shdr) : ByteArray :=
  Gasm.Targets.ELF.serializeElf64Shdr s

/- REF: docs/TARGETS/LINUX.md#31-elf64-layout-header-structure -/
/-- Constructs a null-terminated section header name string table (.shstrtab). -/
...
/- REF: docs/TARGETS/LINUX.md#31-elf64-layout-header-structure -/
/-- Emits a valid, fully formed 64-bit static ELF executable binary image. -/
def emitELF64Executable (baseVma : Address) (textBytes : ByteArray) (rodataBytes : ByteArray) : ByteArray := Id.run do
  ...
  elf := elf ++ serializeProgramHeader phdrText
  elf := elf ++ serializeProgramHeader phdrRodata
  ...
  elf := elf ++ serializeSectionHeader shdrNull
  elf := elf ++ serializeSectionHeader shdrText
  elf := elf ++ serializeSectionHeader shdrRodata
  elf := elf ++ serializeSectionHeader shdrShstrtab
  elf
