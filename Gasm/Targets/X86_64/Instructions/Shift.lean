...
def shiftDecodeRules : List DecodeRule := [
  -- 0xC1 with imm8 (r16, r64, r32)
  -- r16 imm8
  { opcode := .one 0xC1, has0x66 := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (shl_r16_imm8 (codeToReg16 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0xC1, has0x66 := some true, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (shr_r16_imm8 (codeToReg16 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0xC1, has0x66 := some true, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (sar_r16_imm8 (codeToReg16 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  -- r64 imm8
  { opcode := .one 0xC1, has0x66 := some false, rexW := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (shl_r64_imm8 (codeToReg64 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0xC1, has0x66 := some false, rexW := some true, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (shr_r64_imm8 (codeToReg64 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0xC1, has0x66 := some false, rexW := some true, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (sar_r64_imm8 (codeToReg64 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  -- r32 imm8
  { opcode := .one 0xC1, has0x66 := some false, rexW := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (shl_r32_imm8 (codeToReg32 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0xC1, has0x66 := some false, rexW := some false, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (shr_r32_imm8 (codeToReg32 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0xC1, has0x66 := some false, rexW := some false, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (sar_r32_imm8 (codeToReg32 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  -- 0xC0 with imm8 (r8)
  { opcode := .one 0xC0, has0x66 := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (shl_r8_imm8 (codeToReg8 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0xC0, has0x66 := some false, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (shr_r8_imm8 (codeToReg8 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0xC0, has0x66 := some false, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m =>
        ...
        | .error e => .error e
        | .ok imm8 =>
          .ok (sar_r8_imm8 (codeToReg8 m.rm ctx.rexB) imm8, (m.pos + 1) - ctx.startOffset) },
  -- 0xD1 with 1 (r16, r64, r32)
  -- r16 1
  { opcode := .one 0xD1, has0x66 := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shl_r16_one (codeToReg16 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD1, has0x66 := some true, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shr_r16_one (codeToReg16 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD1, has0x66 := some true, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (sar_r16_one (codeToReg16 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  -- r64 1
  { opcode := .one 0xD1, has0x66 := some false, rexW := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shl_r64_one (codeToReg64 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD1, has0x66 := some false, rexW := some true, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shr_r64_one (codeToReg64 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD1, has0x66 := some false, rexW := some true, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (sar_r64_one (codeToReg64 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  -- r32 1
  { opcode := .one 0xD1, has0x66 := some false, rexW := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shl_r32_one (codeToReg32 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD1, has0x66 := some false, rexW := some false, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shr_r32_one (codeToReg32 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD1, has0x66 := some false, rexW := some false, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (sar_r32_one (codeToReg32 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  -- 0xD0 with 1 (r8)
  { opcode := .one 0xD0, has0x66 := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shl_r8_one (codeToReg8 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD0, has0x66 := some false, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shr_r8_one (codeToReg8 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD0, has0x66 := some false, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (sar_r8_one (codeToReg8 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  -- 0xD3 with CL (r16, r64, r32)
  -- r16 CL
  { opcode := .one 0xD3, has0x66 := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shl_r16_cl (codeToReg16 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD3, has0x66 := some true, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shr_r16_cl (codeToReg16 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD3, has0x66 := some true, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (sar_r16_cl (codeToReg16 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  -- r64 CL
  { opcode := .one 0xD3, has0x66 := some false, rexW := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shl_r64_cl (codeToReg64 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD3, has0x66 := some false, rexW := some true, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shr_r64_cl (codeToReg64 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD3, has0x66 := some false, rexW := some true, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (sar_r64_cl (codeToReg64 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  -- r32 CL
  { opcode := .one 0xD3, has0x66 := some false, rexW := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shl_r32_cl (codeToReg32 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD3, has0x66 := some false, rexW := some false, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shr_r32_cl (codeToReg32 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD3, has0x66 := some false, rexW := some false, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (sar_r32_cl (codeToReg32 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  -- 0xD2 with CL (r8)
  { opcode := .one 0xD2, has0x66 := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shl_r8_cl (codeToReg8 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD2, has0x66 := some false, modrmReg := some 5,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (shr_r8_cl (codeToReg8 m.rm ctx.rexB), m.pos - ctx.startOffset) },
  { opcode := .one 0xD2, has0x66 := some false, modrmReg := some 7,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "shift"
      | some m => .ok (sar_r8_cl (codeToReg8 m.rm ctx.rexB), m.pos - ctx.startOffset) }
]
