...
def negDecodeRules : List DecodeRule := [
  { opcode := .one 0xF7,
    ...
    builder := fun ctx =>
      ...
      | none => .error "neg_r16: missing ModR/M byte"
      | some m =>
        let dst := codeToReg16 m.rm ctx.rexB
        .ok (neg_r16 dst, m.pos - ctx.startOffset)
  },
  { opcode := .one 0xF6,
    has0x66 := some false,
    modrmReg := some 3,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "neg_r8: missing ModR/M byte"
      | some m =>
        let dst := codeToReg8 m.rm ctx.rexB
        .ok (neg_r8 dst, m.pos - ctx.startOffset)
  },
  { opcode := .one 0xF7,
    has0x66 := some false,
    rexW := some true,
  ...
  { opcode := .one 0xF7,
    ...
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "neg_r32: missing ModR/M byte"
      | some m =>
        let dst := codeToReg32 m.rm ctx.rexB
        .ok (neg_r32 dst, m.pos - ctx.startOffset)
  }
]
