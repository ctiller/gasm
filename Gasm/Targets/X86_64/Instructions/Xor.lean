...
/-- Co-located decoder for the XOR family: `0x31` (XOR r64/r32/r16), `0x30` (XOR r8), `0x80 /6` (XOR r8, imm8),
    `0x81 /6` (XOR imm32/imm16), and `0x83 /6` (XOR imm8). Errors for any other byte pattern. -/
    ...
    if has0x66 then
      if opcode == 0x31 then
        ...
        | .ok (_, reg, rm, modPos) =>
          if reg == 6 then
            ...
            | .error e => .error e
            | .ok imm8 => .ok (xor_r16_imm8 dst imm8, (modPos + 1) - offset)
          else .error "xorTryDecode: 0x83 sub-opcode is not XOR"
        ...
        | .ok (_, reg, rm, modPos) =>
          if reg == 6 then
            ...
            | .error e => .error e
            | .ok imm16 => .ok (xor_r16_imm16 dst imm16, (modPos + 2) - offset)
          else .error "xorTryDecode: 0x81 sub-opcode is not XOR"
      else .error s!"xorTryDecode: opcode 0x{String.ofList (Nat.toDigits 16 opcode.toNat)} with 0x66 prefix is not XOR"
    else if opcode == 0x30 then
      match readModRM bytes opOffset with
      | .error e => .error e
      | .ok (_, reg, rm, pos) =>
        let dst := codeToReg8 rm rexB
        let src := codeToReg8 reg rexR
        .ok (xor_r8 dst src, pos - offset)
      ...
      | .ok (_, reg, rm, modPos) =>
        if reg == 6 then
          ...
          match readUInt8 bytes modPos with
          | .error e => .error e
          | .ok imm8 => .ok (xor_r8_imm8 dst imm8, (modPos + 1) - offset)
        else .error "xorTryDecode: 0x80 sub-opcode is not XOR"
      ...
      | .ok (_, reg, rm, modPos) =>
        if reg == 6 then
          if rexW then
            ...
            | .error e => .error e
            | .ok imm32 => .ok (xor_r64_imm32 dst imm32, (modPos + 4) - offset)
          else
            let dst := codeToReg32 rm rexB
            match readUInt32LE bytes modPos with
            | .error e => .error e
            | .ok imm32 => .ok (xor_r32_imm32 dst imm32, (modPos + 4) - offset)
        else .error "xorTryDecode: 0x81 sub-opcode is not XOR"
    else if opcode == 0x83 then
      match readModRM bytes opOffset with
      | .error e => .error e
      | .ok (_, reg, rm, modPos) =>
        if reg == 6 then
          if rexW then
            ...
            match readUInt8 bytes modPos with
            | .error e => .error e
            | .ok imm8 => .ok (xor_r64_imm8 dst imm8, (modPos + 1) - offset)
          else
            let dst := codeToReg32 rm rexB
            match readUInt8 bytes modPos with
            | .error e => .error e
            | .ok imm8 => .ok (xor_r32_imm8 dst imm8, (modPos + 1) - offset)
        else .error "xorTryDecode: 0x83 sub-opcode is not XOR"
    else
