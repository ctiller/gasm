...
/- REF: docs/TARGETS/X86_64.md#5-stage-b-decoder-modularization -/
/-- Declarative decoding rules for the OR family: `0x09` (OR r64, r64), `0x81 /1` (OR r64, imm32),
    and `0x83 /1` (OR r64, imm8). -/
def orDecodeRules : List DecodeRule := [
  { opcode := .one 0x09,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "or_r64: missing ModR/M byte"
      | some m =>
  ...
  { opcode := .one 0x81,
    ...
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "or_r64_imm32: missing ModR/M byte"
      | some m =>
        let dst := codeToReg64 m.rm ctx.rexB
        match readUInt32LE ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm32 => .ok (or_r64_imm32 dst imm32, (m.pos + 4) - ctx.startOffset)
  },
  { opcode := .one 0x83,
    modrmReg := some 1,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "or_r64_imm8: missing ModR/M byte"
      | some m =>
        let dst := codeToReg64 m.rm ctx.rexB
        match readUInt8 ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm8 => .ok (or_r64_imm8 dst imm8, (m.pos + 1) - ctx.startOffset)
  }
]
