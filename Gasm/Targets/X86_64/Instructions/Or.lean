...
/-- Co-located decoder for the OR family: `0x09` (OR r64/r32/r16), `0x08` (OR r8), `0x80 /1` (OR r8, imm8),
    `0x81 /1` (OR imm32/imm16), and `0x83 /1` (OR imm8). Errors for any other byte pattern. -/
    ...
    if has0x66 then
      if opcode == 0x09 then
        ...
        | .ok (_, reg, rm, modPos) =>
          if reg == 1 then
            ...
            | .error e => .error e
            | .ok imm8 => .ok (or_r16_imm8 dst imm8, (modPos + 1) - offset)
          else .error "orTryDecode: 0x83 sub-opcode is not OR"
        ...
        | .ok (_, reg, rm, modPos) =>
          if reg == 1 then
            ...
            | .error e => .error e
            | .ok imm16 => .ok (or_r16_imm16 dst imm16, (modPos + 2) - offset)
          else .error "orTryDecode: 0x81 sub-opcode is not OR"
      else .error s!"orTryDecode: opcode 0x{String.ofList (Nat.toDigits 16 opcode.toNat)} with 0x66 prefix is not OR"
      ...
      | .ok (_, reg, rm, modPos) =>
        if reg == 1 then
          ...
          match readUInt8 bytes modPos with
          | .error e => .error e
          | .ok imm8 => .ok (or_r8_imm8 dst imm8, (modPos + 1) - offset)
        else .error "orTryDecode: 0x80 sub-opcode is not OR"
      ...
      | .ok (_, reg, rm, modPos) =>
        if reg == 1 then
          if rexW then
            ...
            match readUInt32LE bytes modPos with
            | .error e => .error e
            | .ok imm32 => .ok (or_r32_imm32 dst imm32, (modPos + 4) - offset)
        else .error "orTryDecode: 0x81 sub-opcode is not OR"
    else if opcode == 0x83 then
      match readModRM bytes opOffset with
      | .error e => .error e
      | .ok (_, reg, rm, modPos) =>
        if reg == 1 then
          if rexW then
            ...
            match readUInt8 bytes modPos with
            | .error e => .error e
            | .ok imm8 => .ok (or_r32_imm8 dst imm8, (modPos + 1) - offset)
        else .error "orTryDecode: 0x83 sub-opcode is not OR"
    else
