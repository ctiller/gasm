...
private def movImmRule (regCode : UInt8) : DecodeRule := {
  ...
  builder := fun ctx =>
    if ctx.has0x66 then
      let dst := codeToReg16 regCode ctx.rexB
      match readUInt16LE ctx.bytes ctx.opcodePos with
      | .error e => .error e
      | .ok imm16 => .ok (mov_r16_imm16 dst imm16, (ctx.opcodePos + 2) - ctx.startOffset)
    else if ctx.rexW then
...
private def movImm8Rule (regCode : UInt8) : DecodeRule := {
  opcode := .one (0xB0 + regCode),
  has0x66 := some false,
  builder := fun ctx =>
    let dst := codeToReg8 regCode ctx.rexB
    match readUInt8 ctx.bytes ctx.opcodePos with
    | .error e => .error e
    | .ok imm8 => .ok (mov_r8_imm8 dst imm8, (ctx.opcodePos + 1) - ctx.startOffset)
}
        ...
        if m.reg != 0 then
          ...
          | .ok sib =>
            if sib != makeSIB 0 4 4 then
              ...
              let nextPos := m.pos + 1
              if m.mod == 0 then
                match readUInt8 ctx.bytes nextPos with
                | .error e => .error e
                | .ok val => .ok (mov_rsp_byte 0 val, (nextPos + 1) - ctx.startOffset)
                ...
                | .ok disp8 =>
                  if disp8 == 0 then
                    ...
                    match readUInt8 ctx.bytes (nextPos + 1) with
                    | .error e => .error e
                    | .ok val => .ok (mov_rsp_byte disp8 val, (nextPos + 2) - ctx.startOffset)
              else
...
def movDecodeRules : List DecodeRule := [
  ...
  { opcode := .one 0x88,
    has0x66 := some false,
    builder := fun ctx =>
      if ctx.rexW || ctx.rexX then
        .error "movTryDecode: unsupported REX.W/REX.X form for 0x88 MOV"
      else
        match ctx.modrm with
        | none => .error "mov 0x88: missing ModR/M byte"
        | some m =>
          if m.mod == 3 then
            ...
            if ctx.hasRex != needsRex then
              ...
              .ok (mov_r8 dst src, m.pos - ctx.startOffset)
          else
            let needsRex := ctx.rexR || ctx.rexB || m.reg >= 4
            if ctx.hasRex != needsRex then
              .error "movTryDecode: noncanonical or legacy high-byte REX identity for 0x88 MOV"
            else
  ...
  { opcode := .one 0x8A,
    ...
    builder := fun ctx =>
      if ctx.rexW || ctx.rexX then
        ...
        match ctx.modrm with
        | none => .error "mov 0x8A: missing ModR/M byte"
        | some m =>
          let needsRex := ctx.rexR || ctx.rexB || m.reg >= 4
          if ctx.hasRex != needsRex then
            ...
            let dstReg := codeToReg8 m.reg ctx.rexR
            ...
            if m.rm == 4 then
              ...
              | .ok sib =>
                if sib == makeSIB 0 4 4 then
                  ...
                  | .error e => .error e
                  | .ok disp8 =>
                    .ok (mov_reg8_mem8_disp dstReg basePtr disp8, (m.pos + 2) - ctx.startOffset)
                else
              ...
              | .error e => .error e
              | .ok disp8 =>
                .ok (mov_reg8_mem8_disp dstReg basePtr disp8, (m.pos + 1) - ctx.startOffset)
  },
  { opcode := .one 0x89,
    has0x66 := some true,
    builder := fun ctx =>
      ...
      | some m =>
        if m.mod == 3 then
          ...
          .ok (mov_r16 dst src, m.pos - ctx.startOffset)
  ...
  { opcode := .one 0x89,
    ...
    builder := fun ctx =>
      ...
      else if !ctx.rexW && ctx.hasRex != (ctx.rexR || ctx.rexB) then
        ...
        | some m =>
          if m.mod == 3 then
            if ctx.rexW then
              ...
              .ok (mov_r32_r32 dst src, m.pos - ctx.startOffset)
            ...
            if ctx.rexW then
              ...
              let srcReg := codeToReg32 m.reg ctx.rexR
              if m.rm == 4 then
                match readUInt8 ctx.bytes m.pos with
                | .error e => .error e
                | .ok sib =>
                  if sib == makeSIB 0 4 4 then
                    let basePtr := codeToReg64 4 ctx.rexB
                    .ok (mov_mem32_disp basePtr 0 srcReg, (m.pos + 1) - ctx.startOffset)
                  else
                ...
                let basePtr := codeToReg64 m.rm ctx.rexB
                .ok (mov_mem32_disp basePtr 0 srcReg, m.pos - ctx.startOffset)
          else if m.mod == 1 then
            if ctx.rexW then
              let srcReg := codeToReg64 m.reg ctx.rexR
              if m.rm == 4 then
              ...
              if m.rm == 4 then
                  ...
                  if sib == makeSIB 0 4 4 then
                    ...
                    | .ok disp8 =>
                      if disp8 == 0 then
                        ...
                        let basePtr := codeToReg64 4 ctx.rexB
                        .ok (mov_mem32_disp basePtr disp8 srcReg, (m.pos + 2) - ctx.startOffset)
                  else
                    .error "movTryDecode: unsupported indexed/noncanonical SIB for 32-bit 0x89 MOV"
              else
                match readUInt8 ctx.bytes m.pos with
                ...
                | .ok disp8 =>
                  if disp8 == 0 && m.rm != 5 then
                    ...
                    let basePtr := codeToReg64 m.rm ctx.rexB
                    .ok (mov_mem32_disp basePtr disp8 srcReg, (m.pos + 1) - ctx.startOffset)
          else
      ...
      if ctx.rexX then
        ...
        | some m =>
          if ctx.rexW then
            ...
            let dstReg := codeToReg32 m.reg ctx.rexR
            if m.mod == 1 then
              if m.rm == 4 then
                match readUInt8 ctx.bytes m.pos with
                | .error e => .error e
                | .ok sib =>
                  if sib == makeSIB 0 4 4 then
                    ...
                    | .ok disp8 =>
                      let basePtr := codeToReg64 4 ctx.rexB
                      .ok (mov_reg32_mem32_disp dstReg basePtr disp8, (m.pos + 2) - ctx.startOffset)
                  else
                ...
                | .ok disp8 =>
                  let basePtr := codeToReg64 m.rm ctx.rexB
                  .ok (mov_reg32_mem32_disp dstReg basePtr disp8, (m.pos + 1) - ctx.startOffset)
            else
  ...
  { opcode := .one 0xC7,
    has0x66 := some false,
    builder := fun ctx =>
      if ctx.rexR || ctx.rexX || (!ctx.rexW && ctx.hasRex) then
        ...
        | some m =>
          if m.reg != 0 then
            ...
            if m.rm == 4 then
              ...
              | .ok sib =>
                if sib != makeSIB 0 4 4 then
                  ...
                  | .ok imm32 =>
                    ...
                    if ctx.rexW then
                      if ctx.rexB then
                        .ok (mov_mem64_disp_imm .r12 0 imm32, pos - ctx.startOffset)
                      else
                        .ok (mov_rsp64 0 imm32, pos - ctx.startOffset)
                    else
                      .ok (mov_rsp32 0 imm32, pos - ctx.startOffset)
            else if m.rm == 5 then
              ...
              | .error e => .error e
              | .ok imm32 =>
                .ok (mov_mem64_disp_imm basePtr 0 imm32, (m.pos + 4) - ctx.startOffset)
          else if m.mod == 1 then
            if m.rm == 4 then
              match readUInt8 ctx.bytes m.pos with
              | .error e => .error e
                  ...
                  | .ok disp8 =>
                    if disp8 == 0 then
                      ...
                      | .ok imm32 =>
                        ...
                        if ctx.rexW then
                          if ctx.rexB then
                            .ok (mov_mem64_disp_imm .r12 disp8 imm32, pos - ctx.startOffset)
                          else
                            .ok (mov_rsp64 disp8 imm32, pos - ctx.startOffset)
                        else
                          .ok (mov_rsp32 disp8 imm32, pos - ctx.startOffset)
            else if !ctx.rexW then
              .error "movTryDecode: unsupported non-RSP dword form for 0xC7 MOV"
            else
              match readUInt8 ctx.bytes m.pos with
                ...
                if disp8 == 0 && m.rm != 5 then
                  ...
                  | .ok imm32 =>
                    .ok (mov_mem64_disp_imm basePtr disp8 imm32,
                      (immPos + 4) - ctx.startOffset)
          else if m.mod == 3 then
            if ctx.rexW then
              let dst := codeToReg64 m.rm ctx.rexB
              match readUInt32LE ctx.bytes m.pos with
              | .error e => .error e
              | .ok imm32 => .ok (mov_r64_imm32 dst imm32, (m.pos + 4) - ctx.startOffset)
            else
  ...
  { opcode := .two0F 0xB6,
    ...
    builder := fun ctx =>
      if ctx.rexX then
        ...
        | some m =>
          match decodeMovzxMem8Address ctx.bytes m.mod m.rm m.pos ctx.rexB with
          | .error e => .error e
          | .ok (basePtr, disp8, nextPos) =>
            if ctx.rexW then
              .ok (movzx_r64_mem8 (codeToReg64 m.reg ctx.rexR) basePtr disp8, nextPos - ctx.startOffset)
            else
              .ok (movzx_r32_mem8 (codeToReg32 m.reg ctx.rexR) basePtr disp8, nextPos - ctx.startOffset)
  }
]
