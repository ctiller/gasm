...
private def cmovRule (op : UInt8) (mk : Reg64 → Reg64 → AnyX86_64Instruction) : DecodeRule := {
  opcode := .two0F op,
  builder := fun ctx =>
    match ctx.modrm with
    | none => .error "cmov: missing ModR/M byte"
    | some m =>
      let dst := codeToReg64 m.reg ctx.rexR
      let src := codeToReg64 m.rm ctx.rexB
      .ok (mk dst src, m.pos - ctx.startOffset)
}

/- REF: docs/TARGETS/X86_64.md#5-stage-b-decoder-modularization -/
/-- Declarative decoding rules for the CMOV family. -/
def cmovDecodeRules : List DecodeRule := [
  cmovRule 0x42 cmovb_r64,
  cmovRule 0x43 cmovae_r64,
  cmovRule 0x44 cmove_r64,
  cmovRule 0x45 cmovne_r64,
  cmovRule 0x4C cmovl_r64,
  cmovRule 0x4D cmovge_r64,
  cmovRule 0x4E cmovle_r64,
  cmovRule 0x4F cmovg_r64
]

/- REF: docs/TARGETS/X86_64.md#5-stage-b-decoder-modularization -/
/-- Co-located decoder for the CMOV family, evaluating its declarative rules. -/
def cmovTryDecode (bytes : ByteArray) (offset : Nat) : Except String (AnyX86_64Instruction × Nat) :=
  tryDecodeWithRules cmovDecodeRules bytes offset
