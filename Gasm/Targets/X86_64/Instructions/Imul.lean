...
def imulDecodeRules : List DecodeRule := [
  -- 0x0F 0xAF (2-operand)
  { opcode := .two0F 0xAF,
    has0x66 := some true,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r16: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        let dst := codeToReg16 m.reg ctx.rexR
        let src := codeToReg16 m.rm ctx.rexB
        .ok (imul_r16 dst src, m.pos - ctx.startOffset)
  },
  { opcode := .two0F 0xAF,
    has0x66 := some false,
    rexW := some true,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r64: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        let dst := codeToReg64 m.reg ctx.rexR
        let src := codeToReg64 m.rm ctx.rexB
        .ok (imul_r64 dst src, m.pos - ctx.startOffset)
  },
  { opcode := .two0F 0xAF,
    has0x66 := some false,
    rexW := some false,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r32: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        let dst := codeToReg32 m.reg ctx.rexR
        let src := codeToReg32 m.rm ctx.rexB
        .ok (imul_r32 dst src, m.pos - ctx.startOffset)
  },
  -- 0x69 (3-operand imm32/imm16)
  { opcode := .one 0x69,
    has0x66 := some true,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r16_r16_imm16: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        ...
        | .ok imm =>
          let dst := codeToReg16 m.reg ctx.rexR
          let src := codeToReg16 m.rm ctx.rexB
          .ok (imul_r16_r16_imm16 dst src imm, (m.pos + 2) - ctx.startOffset)
  },
  { opcode := .one 0x69,
    has0x66 := some false,
    rexW := some true,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r64_r64_imm32: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        ...
        | .ok imm =>
          let dst := codeToReg64 m.reg ctx.rexR
          let src := codeToReg64 m.rm ctx.rexB
          .ok (imul_r64_r64_imm32 dst src imm, (m.pos + 4) - ctx.startOffset)
  },
  { opcode := .one 0x69,
    has0x66 := some false,
    rexW := some false,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r32_r32_imm32: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        match readUInt32LE ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm =>
          let dst := codeToReg32 m.reg ctx.rexR
          let src := codeToReg32 m.rm ctx.rexB
          .ok (imul_r32_r32_imm32 dst src imm, (m.pos + 4) - ctx.startOffset)
  },
  -- 0x6B (3-operand imm8)
  { opcode := .one 0x6B,
    has0x66 := some true,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r16_r16_imm8: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        ...
        | .ok imm =>
          let dst := codeToReg16 m.reg ctx.rexR
          let src := codeToReg16 m.rm ctx.rexB
          .ok (imul_r16_r16_imm8 dst src imm, (m.pos + 1) - ctx.startOffset)
  },
  { opcode := .one 0x6B,
    has0x66 := some false,
    rexW := some true,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r64_r64_imm8: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        match readUInt8 ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm =>
          let dst := codeToReg64 m.reg ctx.rexR
          let src := codeToReg64 m.rm ctx.rexB
          .ok (imul_r64_r64_imm8 dst src imm, (m.pos + 1) - ctx.startOffset)
  },
  { opcode := .one 0x6B,
    has0x66 := some false,
    rexW := some false,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "imul_r32_r32_imm8: missing ModR/M byte"
      | some m => if m.mod != 3 then .error "imulTryDecode: memory operand unsupported" else
        match readUInt8 ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm =>
          let dst := codeToReg32 m.reg ctx.rexR
          let src := codeToReg32 m.rm ctx.rexB
          .ok (imul_r32_r32_imm8 dst src imm, (m.pos + 1) - ctx.startOffset)
  }
]

/- REF: docs/TARGETS/X86_64.md#5-stage-b-decoder-modularization -/
/-- Co-located decoder for the IMUL family, evaluating its declarative rules. -/
def imulTryDecode (bytes : ByteArray) (offset : Nat) : Except String (AnyX86_64Instruction × Nat) :=
  tryDecodeWithRules imulDecodeRules bytes offset
