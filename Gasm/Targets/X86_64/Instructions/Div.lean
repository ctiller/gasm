...
/-- Co-located decoder for the DIV/IDIV/MUL family: Group 3 opcodes `0xF6` and `0xF7` with
    ModRM digits /4 (MUL), /6 (DIV), /7 (IDIV). Errors for any other opcode or ModRM digit. -/
  ...
  | .ok (has0x66, _, rexW, _, _, rexB, opcode, opOffset) =>
    if has0x66 then
      if opcode == 0xF7 then
        ...
        | .error e => .error e
        | .ok (mod, reg, rm, pos) =>
          if mod != 3 then .error "divTryDecode: memory operand unsupported"
          else
            let r := codeToReg16 rm rexB
            if reg == 6 then .ok (div_r16 r, pos - offset)
            else if reg == 7 then .ok (idiv_r16 r, pos - offset)
            else if reg == 4 then .ok (mul_r16 r, pos - offset)
            else .error "divTryDecode: 0xF7 sub-opcode is not DIV/IDIV/MUL"
      else .error "divTryDecode: opcode with 0x66 prefix is not DIV/IDIV/MUL"
      ...
      | .ok (mod, reg, rm, pos) =>
        if mod != 3 then .error "divTryDecode: memory operand unsupported"
          ...
          let r := codeToReg8 rm rexB
          if reg == 6 then .ok (div_r8 r, pos - offset)
          else if reg == 7 then .ok (idiv_r8 r, pos - offset)
          else if reg == 4 then .ok (mul_r8 r, pos - offset)
          else .error "divTryDecode: 0xF6 sub-opcode is not DIV/IDIV/MUL"
      ...
      | .ok (mod, reg, rm, pos) =>
        if mod != 3 then .error "divTryDecode: memory operand unsupported"
          ...
          let r := codeToReg64 rm rexB
          if reg == 6 then .ok (div_r64 r, pos - offset)
          else if reg == 7 then .ok (idiv_r64 r, pos - offset)
          else if reg == 4 then .ok (mul_r64 r, pos - offset)
          else .error "divTryDecode: 0xF7 sub-opcode is not DIV/IDIV/MUL"
        else
          let r := codeToReg32 rm rexB
          if reg == 6 then .ok (div_r32 r, pos - offset)
          else if reg == 7 then .ok (idiv_r32 r, pos - offset)
          else if reg == 4 then .ok (mul_r32 r, pos - offset)
          else .error "divTryDecode: 0xF7 sub-opcode is not DIV/IDIV/MUL"
    else
      .error s!"divTryDecode: opcode 0x{String.ofList (Nat.toDigits 16 opcode.toNat)} is not DIV/IDIV/MUL"
