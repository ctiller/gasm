...
def andDecodeRules : List DecodeRule := [
  -- 0x21 (reg, reg)
  { opcode := .one 0x21, has0x66 := some true,
    builder := fun ctx =>
      ...
      | some m =>
        ...
        .ok (and_r16 dst src, m.pos - ctx.startOffset) },
  { opcode := .one 0x21, has0x66 := some false, rexW := some true,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r64"
      | some m =>
  ...
  { opcode := .one 0x21, has0x66 := some false, rexW := some false,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r32"
      | some m =>
        ...
        .ok (and_r32 dst src, m.pos - ctx.startOffset) },
  -- 0x20 (r8, r8)
  { opcode := .one 0x20, has0x66 := some false,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r8"
      | some m =>
        ...
        .ok (and_r8 dst src, m.pos - ctx.startOffset) },
  -- 0x80 /4 (r8, imm8)
  { opcode := .one 0x80, has0x66 := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r8_imm8"
      | some m =>
        match readUInt8 ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm8 =>
          let dst := codeToReg8 m.rm ctx.rexB
          .ok (and_r8_imm8 dst imm8, (m.pos + 1) - ctx.startOffset) },
  -- 0x81 /4 (imm)
  { opcode := .one 0x81, has0x66 := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r16_imm16"
      | some m =>
        match readUInt16LE ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm16 =>
          let dst := codeToReg16 m.rm ctx.rexB
          .ok (and_r16_imm16 dst imm16, (m.pos + 2) - ctx.startOffset) },
  { opcode := .one 0x81, has0x66 := some false, rexW := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r64_imm32"
      | some m =>
        match readUInt32LE ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm32 =>
          let dst := codeToReg64 m.rm ctx.rexB
          .ok (and_r64_imm32 dst imm32, (m.pos + 4) - ctx.startOffset) },
  { opcode := .one 0x81, has0x66 := some false, rexW := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r32_imm32"
      | some m =>
        match readUInt32LE ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm32 =>
          let dst := codeToReg32 m.rm ctx.rexB
          .ok (and_r32_imm32 dst imm32, (m.pos + 4) - ctx.startOffset) },
  -- 0x83 /4 (imm8)
  { opcode := .one 0x83, has0x66 := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r16_imm8"
      | some m =>
        match readUInt8 ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm8 =>
          let dst := codeToReg16 m.rm ctx.rexB
          .ok (and_r16_imm8 dst imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0x83, has0x66 := some false, rexW := some true, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r64_imm8"
      | some m =>
        match readUInt8 ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm8 =>
          let dst := codeToReg64 m.rm ctx.rexB
          .ok (and_r64_imm8 dst imm8, (m.pos + 1) - ctx.startOffset) },
  { opcode := .one 0x83, has0x66 := some false, rexW := some false, modrmReg := some 4,
    builder := fun ctx =>
      match ctx.modrm with
      | none => .error "and_r32_imm8"
      | some m =>
        match readUInt8 ctx.bytes m.pos with
        | .error e => .error e
        | .ok imm8 =>
          let dst := codeToReg32 m.rm ctx.rexB
          .ok (and_r32_imm8 dst imm8, (m.pos + 1) - ctx.startOffset) }
]
