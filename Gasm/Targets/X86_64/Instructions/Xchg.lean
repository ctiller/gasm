...
/-- Co-located decoder for the XCHG family: `0x87` (XCHG r64/r32/r16), `0x86` (XCHG r8), `0x90` (NOP).
...
def xchgTryDecode (bytes : ByteArray) (offset : Nat) : Except String (AnyX86_64Instruction × Nat) :=
  ...
  | .error e => .error e
  | .ok (has0x66, hasRex, rexW, rexR, _, rexB, opcode, opOffset) =>
    if has0x66 then
      if opcode == 0x87 then
        ...
        | .ok (mod, reg, rm, pos) =>
          if mod == 3 then
            ...
            .ok (xchg_r16 dst src, pos - offset)
          else .error "xchgTryDecode: unsupported memory form for 16-bit XCHG"
      else .error s!"xchgTryDecode: opcode 0x{String.ofList (Nat.toDigits 16 opcode.toNat)} with 0x66 prefix is not XCHG"
    else if opcode == 0x90 && !hasRex then
      .ok (nop_op, 1)
    else if opcode == 0x86 then
      match readModRM bytes opOffset with
      | .error e => .error e
      | .ok (mod, reg, rm, pos) =>
        if mod == 3 then
      ...
      | .ok (mod, reg, rm, pos) =>
        if mod == 3 then
          if rexW then
            ...
            .ok (xchg_r32 dst src, pos - offset)
