...
/-- Co-located decoder for the TEST family: `0x85` (TEST r64/r32/r16), `0x84` (TEST r8), `0xF6 /0` (TEST r8, imm8),
    and `0xF7 /0` (TEST imm32/imm16). Errors for any other byte pattern. -/
    ...
    if has0x66 then
      if opcode == 0x85 then
        ...
        | .ok (_, reg, rm, pos) =>
          ...
          .ok (test_r16 dst src, pos - offset)
        ...
        | .ok (_, reg, rm, modPos) =>
          if reg == 0 then
            ...
            | .error e => .error e
            | .ok imm16 => .ok (test_r16_imm16 dst imm16, (modPos + 2) - offset)
          else .error "testTryDecode: 0xF7 sub-opcode is not TEST"
      else .error s!"testTryDecode: opcode 0x{String.ofList (Nat.toDigits 16 opcode.toNat)} with 0x66 prefix is not TEST"
      ...
      | .ok (_, reg, rm, modPos) =>
        if reg == 0 then
          ...
          | .error e => .error e
          | .ok imm8 => .ok (test_r8_imm8 dst imm8, (modPos + 1) - offset)
        else .error "testTryDecode: 0xF6 sub-opcode is not TEST"
      ...
      | .ok (_, reg, rm, pos) =>
        if rexW then
          ...
          .ok (test_r32 dst src, pos - offset)
    else if opcode == 0xF7 then
      match readModRM bytes opOffset with
      | .error e => .error e
      | .ok (_, reg, rm, modPos) =>
        if reg == 0 then
          if rexW then
            ...
            match readUInt32LE bytes modPos with
            | .error e => .error e
            | .ok imm32 => .ok (test_r32_imm32 dst imm32, (modPos + 4) - offset)
        else .error "testTryDecode: 0xF7 sub-opcode is not TEST"
    else
