...
-- DIV is register-only (`memAccesses _ := []`): see MemoryFrame/Add.lean's header comment for the
...
-- below discharges those branches the same way it does every other conditional register-only
-- form's `step`.
