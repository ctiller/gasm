...
-- NEG is register-only (`memAccesses _ := []`): see MemoryFrame/Add.lean's header comment for the
-- batch-lemma-instantiation rationale (identical here).

/- REF: docs/MEMORY_HOOK.md#33-the-declarative-access-descriptor-the-one-source-four-consumers-read -/
theorem NegR64.writesWithin (i : NegR64) : WritesWithin i :=
  registerOnly_writesWithin i (fun s => by
    simp only [X86_64Instruction.step, X86_64MachineState.setGpr64,
      X86_64MachineState.setFlagsNeg64] <;>
    (try split) <;> (try split) <;> (try split) <;> (try split) <;> rfl)

/- REF: docs/MEMORY_HOOK.md#33-the-declarative-access-descriptor-the-one-source-four-consumers-read -/
theorem NegR64.readsWithin (i : NegR64) : ReadsWithin i :=
