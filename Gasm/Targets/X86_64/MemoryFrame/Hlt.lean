...
-- HLT is register-only (`memAccesses _ := []`): `step` only advances RIP and records `.halted`;
-- it never touches `.memory`. See MemoryFrame/Add.lean's header comment for the
-- batch-lemma-instantiation rationale (identical here).

/- REF: docs/MEMORY_HOOK.md#33-the-declarative-access-descriptor-the-one-source-four-consumers-read -/
theorem HltOp.writesWithin (i : HltOp) : WritesWithin i :=
  registerOnly_writesWithin i (fun s => by
    simp only [X86_64Instruction.step] <;>
    (try split) <;> (try split) <;> (try split) <;> (try split) <;> rfl)

/- REF: docs/MEMORY_HOOK.md#33-the-declarative-access-descriptor-the-one-source-four-consumers-read -/
theorem HltOp.readsWithin (i : HltOp) : ReadsWithin i :=
