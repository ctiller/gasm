...
/-
Gasm/Targets/X86_64/PerfHardwareFuzzer.lean - hardware-backed perf fuzzer. Reuses
...
/-- One kernel's fully-reduced measurement: raw samples preserved (pre-subtraction, run order),
    ...
    calls for. `perBracketMedian`/`Min`/`Max` are timer-overhead-subtracted totals for the emitted
    `kernelUnrollPerRep`-copy bracket and are the values checked against equivalently replicated
    model bounds. `cyclesPerInstanceMedian` is a divided, reporting-only estimate; containment
    never compares that `Float` with bracket-level `Nat` bounds. -/
...
/-- Model bounds for a kernel tied to a real instruction: `kernelUnrollPerRep` copies of that
    instruction's own `toUops`, exactly matching the number of instruction copies in the emitted
    bracket. The resulting bounds and `ReducedTiming.perBracketMedian` therefore have the same
    unit: cycles per 512-copy bracket. -/
...
/- REF: docs/RDTSC_HARNESS.md#8-the-promotion-rule-this-document-follows-for-costprovenance -/
/-- Containment: the real timer-overhead-subtracted cycles per bracket fall within the model
    bounds for the identically replicated uop stream. -/
...
/-- Inclusive positive-control band in cycles per emitted bracket. The upper endpoint preserves
    the original generous 60-cycles-per-instance ceiling while the lower endpoint rejects a
    zero/stuck timer without incorrectly requiring a throughput kernel to take ≥1 cycle per
    individual NOP. -/
def positiveControlBandCyclesPerBracket : Int × Int :=
  (1, Int.ofNat (60 * kernelUnrollPerRep))

/- REF: docs/RDTSC_HARNESS.md#4-containment-fail-closed-world-sampling-vs-correctness-unrepresentable-by-construction -/
def positiveControlInBand (measuredCyclesPerBracket : Int) : Bool :=
  let (lo, hi) := positiveControlBandCyclesPerBracket
  measuredCyclesPerBracket >= lo && measuredCyclesPerBracket <= hi

/- REF: docs/RDTSC_HARNESS.md#4-containment-fail-closed-world-sampling-vs-correctness-unrepresentable-by-construction -/
/-- Compile-checked negative control for the exact regression this band is meant to prevent. -/
theorem positiveControlRejectsZero : positiveControlInBand 0 = false := by
  rfl
...
/-- Minimum per-bracket cycle delta `long_dependent_chain` must measure above `nop_loop` for the
    ...
    `docs/CALIBRATION_GOVERNANCE.md` §4 check 4 and §8 for why this control exists (wsc's actual observed
...
def verifyTimingOracleControls (overhead nop chain : ReducedTiming) : TimingControlResult :=
  let (lo, hi) := positiveControlBandCyclesPerBracket
  if !positiveControlInBand nop.perBracketMedian then
    { ok := false, detail := s!"POSITIVE CONTROL FAILED: nop_loop measured {nop.perBracketMedian} cycles/bracket after overhead subtraction, outside the inclusive expected band [{lo}, {hi}] -- the timing oracle is not trustworthy (zero/stuck timer, broken bracket, or a machine too noisy to measure on)." }
    ...
    { ok := true, detail := s!"positive control (nop_loop={nop.perBracketMedian} cyc/bracket in inclusive band [{lo},{hi}]) and discrimination pair (long_dependent_chain - nop_loop = {chain.perBracketMedian - nop.perBracketMedian} cyc/bracket, floor {discriminationMinDeltaCycles}) both passed." }
...
/- REF: docs/CALIBRATION_GOVERNANCE.md#2-provenance-and-run-conditions -/
...
/- REF: docs/CALIBRATION_GOVERNANCE.md#5-reduction-and-the-timer-overhead-dag -/
...
def buildCalibrationJson (prov : RunProvenance) (r : ReducedTiming) (timerOverheadRef : Option String)
    ...
    (positiveCyclesBandPerBracket : Int × Int) (positiveMeasuredPerBracket : Int)
  ...
  let runConditions := Json.mkObj [
    ...
    ("core_affinity", ("process-wide mask 0x1 requested by the emitted SetProcessAffinityMask(current-process) call; the BOOL result is not captured, so application is unverified" : Json)),
  ...
  let (band0, band1) := positiveCyclesBandPerBracket
  let controlsBase := [
    ("positive_control_kernel", ("nop_loop" : Json)),
    ("positive_control_unit", (s!"timer-overhead-subtracted cycles per {kernelUnrollPerRep}-copy bracket" : Json)),
    ("positive_control_measured_cycles", (positiveMeasuredPerBracket : Json)),
  ...
  let reduction := Json.mkObj [
    ("method", (s!"median-of-{measuredRepetitions} over raw_samples_cycles_unadjusted (per-bracket total across kernelUnrollPerRep={kernelUnrollPerRep} instances; a discarded first pass executes the same bracket bytes at the same addresses before the retained pass), minus timer_overhead.json's own current median, after {warmupIterations} straight-line-unrolled warm-up instances (docs/RDTSC_HARNESS.md #6)" : Json)),
...
def buildCalibrationMdStub (name jsonRelPath description : String) : String :=
  s!"# {name} -- cycle-measurement evidence (Golden Cove profile, provisional)\n\n" ++
  s!"{description}\n\n" ++
  ...
  "**Status: provisional.** `python scripts/check_calibration.py` (specified by " ++
  "`docs/CALIBRATION_GOVERNANCE.md` but not yet implemented) does not exist yet to " ++
...
/- REF: docs/RDTSC_HARNESS.md#11-vacuity-floor-composition -/
...
def runHardwareFuzz (seed : UInt64) : IO UInt32 := do
  IO.println "--------------------------------------------------------------------------------"
  IO.println " RDTSC/RDTSCP HARDWARE MEASUREMENT (docs/RDTSC_HARNESS.md)"
  IO.println "--------------------------------------------------------------------------------"
  IO.println "[!] VALIDITY: only on real, non-virtualized silicon (docs/RDTSC_HARNESS.md #7)."
  IO.println "    Never valid on a hosted CI runner; ordinary model-only perf_fuzzer mode remains CI-safe (docs/CI.md #5)."
  ...
  if suite.isEmpty then
    IO.println "[VACUITY FLOOR TRIPPED] Zero kernels in the hardware-timed suite -- 0 vectors exercised verifies nothing (docs/REVIEW.md Law 13)."
    return 1
  ...
  | .ok raws =>
    ...
    | some overheadRaw, some nopRaw, some chainRaw =>
      ...
      let mut containmentCandidates : List String := []
      ...
      for entry in suite do
        ...
        | some raw =>
          ...
          | some instr =>
            ...
            IO.println s!"  {entry.kernel.name} ({X86_64Instruction.toNASM instr}): perBracketMedian={reduced.perBracketMedian} (min={reduced.perBracketMin},max={reduced.perBracketMax}{iqrNote}) vs replicatedModelBracket[{bounds.minCycles},{bounds.nominalCycles},{bounds.maxCycles}] -> {verdict}; derivedCyclesPerInstance~{reduced.cyclesPerInstanceMedian} [reporting only]"
            rankable := rankable ++ [{ name := entry.kernel.name, modelNominal := bounds.nominalCycles, realMedian := reduced.perBracketMedian }]
            if contained then
              containmentCandidates := containmentCandidates ++ [entry.kernel.name]
      ...
      | some candRaw, some candInstr =>
        ...
        if !miscalResult.ok then
          ...
          return 1
      | _, _ =>
        IO.println "[MIS-CAL CONTROL] SKIPPED (soft warning, not fatal): no kernel in this run's suite honestly contained within its model bounds, so there is no clean base case to demonstrate a hold-then-break transition against. This reflects real measurement dispersion on a shared machine with only an unverified process-affinity request (docs/RDTSC_HARNESS.md #7), not a harness defect -- the discrimination-pair control above is the primary oracle-trustworthiness gate and it passed."
      ...
      let writeKernel (r : ReducedTiming) (timerRef : Option String) (subject : List String) (bindings : List (String × String × String)) (discPair : Option (String × String × Int × Int)) : IO Unit := do
        let j := buildCalibrationJson prov r timerRef subject bindings positiveControlBandCyclesPerBracket nopReduced.perBracketMedian discPair
      ...
      IO.println s!"\n[*] Wrote {suite.length} calibration file(s) under calibration/x86_64/ (provisional pending the calibration-governance gate -- docs/RDTSC_HARNESS.md #12)."
      IO.println s!"[*] Kernels whose real measurement contained within the current model's bounds ({containmentCandidates.length}/{containmentTotal}): {containmentCandidates}"
      IO.println "    Per docs/RDTSC_HARNESS.md #8, these are candidates for calibration-governance assessment only. Containment is necessary but not sufficient for citation; an artifact must also pass the future mechanical gate and be accepted and bound to the coefficient."
      IO.println "--------------------------------------------------------------------------------"
      return 0
    | _, _, _ =>
