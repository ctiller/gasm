...
/-!
# x86-64 decimal one-pass contracts
...
/-- Architectural effects are derived from the pass safety certificates; storing this large
dependent proposition in every pass record only makes the kernel check the same evidence twice. -/
theorem effect {Event : Type} [interceptor : ExternalCallInterceptor X86_64 Event]
    {selected : Gasm.Core.Address → X86_64MachineState → Bool}
    {indexed : List (UInt64 × X86_64Instr)} {backDisp : UInt8} {stackLower : UInt64}
    {initial : X86_64MachineState}
    (pass : SelectedExtractionPass (Event := Event) selected indexed backDisp stackLower initial) :
    ExtractionPassEffect backDisp initial (extractionFinal backDisp initial) :=
  extractionPassEffect backDisp stackLower initial pass.safety pass.executionSafety
...
/-- Write effects are likewise a theorem of the stored safety certificates, not independent
evidence that every producer should materialize. -/
theorem effect {Event : Type} [interceptor : ExternalCallInterceptor X86_64 Event]
    {selected : Gasm.Core.Address → X86_64MachineState → Bool}
    {indexed : List (UInt64 × X86_64Instr)} {backDisp : UInt8}
    ...
    WritePassEffect backDisp initial (writeFinal backDisp initial) :=
  writePassEffect backDisp stackUpper outputLimit initial pass.safety pass.executionSafety
