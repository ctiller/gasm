...
open Gasm.Targets.Wasm.SemanticsFuzzer

/- REF: wasm-syntax-instructions#instructions -/
/-- CLI Driver for WebAssembly Host Semantic Fuzzer. -/
def main (args : List String) : IO UInt32 := do
  let mut iterations : Nat := 50
  ...
  let (_passed, failed, _) ← runWasmSemanticsFuzzerSuite iterations seed instrFilter
  if failed == 0 then
    return 0
  else
    return 1
