...
## T14 — `bv_decide`'s native-eval axiom + bundled external SAT solver (added post-PA1, 2026-08-27)
PA1 (crc32 pathfinder) introduced this codebase's first `bv_decide` proofs (`Stdlib/Zlib/CRC32Equivalence.lean`: `and_one_cases`, `G_eq_Gbf`, `xor_byte_shr8`, `G8bf_table`, plus the downstream `G8_eq_Gbf8`/`crc32ByteStep_eq_G8`). Task-scoped investigation established, empirically:

- **Axioms actually introduced** (`#print axioms`, toolchain v4.33.1, this tree): each `bv_decide` call synthesizes its own declaration-local axiom, e.g. `and_one_cases._native.bv_decide.ax_1_5`, `G8bf_table._native.bv_decide.ax_1_5` — structurally identical in shape to `native_decide`'s `<decl>._native.native_decide.ax_i_j`. `crc32ByteStep_eq_G8` (the Law 12 connection theorem PA1 exists to exercise) transitively depends on FIVE such axioms (one each from `G8bf_table`, `xor_byte_shr8`, `and_one_cases`, and two from `G_eq_Gbf`'s two case-split calls) plus the three standard ones — zero `sorryAx`, zero hand-declared axioms.
- **NOT kernel-checked — same trust class as `native_decide`, confirmed by reading the mechanism, not just its name.** `Lean/Meta/Tactic/BVDecide/Prover/Bitblast.lean`'s `LratCert.toReflectionProof` builds a term `verifyBVExpr reflectedExpr certExpr` (the LRAT-certificate checker applied to this proof's own reflected goal and certificate) and passes it to `Lean.Meta.nativeEqTrue` — the literal same function `native_decide` itself calls (`Lean/Meta/Native.lean`; its own doc comment: "infrastructure for proofs by native evaluation (`native decide`, `bv_decide`)... asserting the result of that computation as an axiom"). `nativeEqTrue` compiles the boolean expression, RUNS it as native code, and if `true`, asserts an axiom — the kernel never sees or replays the LRAT certificate itself. This toolchain (v4.33.1) has no `bv_decide (config := {checkProofs := true})`-style kernel-replay option (searched: `Lean/Meta/Tactic/BVDecide/TacticContext.lean` and the whole `BVDecide` tree have no `checkProofs` field at all — the only `checkProofs` in the toolchain is an unrelated Grind-tactic internal). **Conclusion: `bv_decide` is categorically the SAME trust class as `native_decide`, not a stronger, kernel-checked one** — the SAT/LRAT step only produces the evidence that gets trusted by compiled execution, exactly like `native_decide`'s brute-force table scan.
- **New external-binary TCB entry, distinct in kind from T9**: `bv_decide` spawns an external CaDiCaL-compatible SAT solver process (`Lean/Meta/Tactic/BVDecide/External.lean`'s `satQuery`, via `IO.Process.spawn`) to search for the CNF/LRAT certificate before the native-eval step above trusts it. `TacticContext.lean`'s `determineSolver` resolves this to `cadical.exe` shipped in the SAME directory as the running `lean.exe` (confirmed present at `<toolchain>/bin/cadical.exe` in this installation), falling back to a bare `"cadical"` on `PATH` if that file is absent. **This differs from T9's unpinned oracles**: `cadical.exe` ships INSIDE the `leanprover/lean4:v4.33.1` toolchain release itself, so it is pinned by the same `lean-toolchain` file mechanism as `lean.exe`/`lake.exe` are (T1) — a toolchain bump changes it atomically with everything else T1 already tracks, not independently and silently the way T9's `node`/`python`/`nasm` can drift. It is still a genuinely new dependency to disclose: the SAT solver's search process is untrusted (a wrong/incomplete search just fails to find a certificate — no soundness risk from THAT half), but its version is not currently asserted or printed by any gate output the way TC5 is planned to do for T9's oracles, and the PATH fallback path (if the bundled binary is ever absent) is unpinned by construction.
- **What this does NOT change**: `Tools/CheckGatesAxioms.lean`'s `hasNativeComponent` (matches any `_native` name component structurally, not a fixed substring) already caught every `bv_decide` axiom before this investigation — confirmed by `lake exe check_gates_axioms` exiting 0 with 80/80 non-standard-axiom-dependent declarations allowlisted, 0 offenders. The load-bearing gate had no blind spot here; only `scripts/check_gates.py`'s source-text pre-check needed teaching the spelling (done, same task, see that script's own commit).
- **Verified vs. inferred**: the axiom names, the `nativeEqTrue`/`LratCert.toReflectionProof` call chain, the absence of a `checkProofs` config, and the bundled `cadical.exe` path were all confirmed directly (empirical `#print axioms` run + reading the shipped toolchain's own `Lean`/`Std` source under `src/lean/Lean/Meta/Tactic/BVDecide/` + a `lake exe check_gates_axioms` run in this tree, commit `0ebf743`). Not independently verified: `cadical`'s own internal correctness (this ledger takes Lean core's choice of solver on faith, same as any other T1-adjacent toolchain component); whether `nativeEqTrue`'s doc-comment phrase "using the Lean kernel" is a typo for "as opposed to the Lean kernel" or means something else was not asked upstream — the surrounding code and the axiom mechanism are the load-bearing evidence, not that one sentence.
- **Recommendation left to the owner, not ratified here**: see the proposed Law 10 amendment in `docs/REVIEW.md` (marked `**Status**: proposed`). Fix if adopted: (i) gate output records the resolved solver path/version, mirroring TC5's plan for T9; (ii) Law 10's text names `bv_decide` explicitly alongside `native_decide`/`decide` rather than leaving the SAT-solver case to be inferred from "the same restriction, different mechanism." Priority: 5 (tie with T11-b-style disclosure gaps — real, not urgent; nothing here is unsound, it is undisclosed). Shrinkable to a disclosure/wording fix; the underlying trust boundary (compiled execution + an external, but pinned, SAT solver) is not itself reducible without Lean core adding a kernel-replay path.
- **RATIFIED 2026-08-27** (`PLAN.md` D28; [ADR-0037](docs/adr/0037-ratify-bv-decide-trust-tier.md)): the owner approved `bv_decide`'s admissibility under Law 10's existing restriction ("ok, bv_decide seems fine then"). `docs/REVIEW.md` Law 10 now presents an explicit four-rung trust-cost ordering (structural proof / `decide` / `native_decide` / `bv_decide`) rather than a bare list, per this entry's finding that rung 2 is categorically different (genuinely kernel-checked) from rungs 3–4 (trusted, not checked). Both of this entry's recommended fixes are done: (i) `scripts/run_gates.py`'s `detect_cadical()` records the resolved solver path/version in the same oracle-version table as T9's `node`/`nasm`/`python`; (ii) Law 10's text names `bv_decide` explicitly. The four `finite-forall` allowlist entries are unchanged, per this entry's own recommendation.
