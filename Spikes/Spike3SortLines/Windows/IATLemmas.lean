...
2. **`computeSectionLayout`'s section-disjointness is `alignUp`-generic.** `PEFormat.lean`'s
   ...
   `.rdataBytes.size` as cheap literals (`1540`, `2`), `computeSectionLayout 1540 2 512` itself
...
/- REF: docs/TARGETS/WINDOWS.md#1-microsoft-x64-calling-convention -/
/-- Spike 3's assembled `.text` section is exactly 1540 bytes. Proved via `foldl_append_size`
...
theorem spike3_textBytes_size : spike3Executable.textBytes.size = 1540 := by
  show (Assembler.serializeInstructions spike3Instructions).size = 1540
...
/- REF: docs/TARGETS/WINDOWS.md#1-microsoft-x64-calling-convention -/
/-- Spike 3's concrete section layout (`computeSectionLayout 1540 2 512`) places `.text` at RVA
