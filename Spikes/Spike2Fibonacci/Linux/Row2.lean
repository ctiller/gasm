...
def spike2Row2AfterIndexHeader : X86_64MachineState :=
        ...
        (X86_64Instruction.step (mov_rsp_byte 0x42 0x62)
          (X86_64Instruction.step (mov_rsp_byte 0x41 0x69)
            (X86_64Instruction.step (mov_rsp_byte 0x40 0x46)
              (spike2AfterMainHeader spike2Row1AfterRecurrence))))))

/-- The real joined value-format entry reached by the row-one one-digit branch. -/
def spike2Row2AfterIndex : X86_64MachineState :=
        ...
        (X86_64Instruction.step (push_r64 .rdx)
          (X86_64Instruction.step (add_r64_imm8 .rdx 0x30)
            (X86_64Instruction.step (div_r64 .r10)
              (X86_64Instruction.step (xor_r32 .edx .edx) spike2Row2AfterValueSetup))))))

/-- The terminating pop/write pass which materializes row one's sole decimal digit. -/
def spike2Row2AfterWrite : X86_64MachineState :=
    ...
    (X86_64Instruction.step (sub_r64_imm8 .rcx 1)
      (X86_64Instruction.step (add_r64_imm8 .rdi 1)
        (X86_64Instruction.step (mov_mem8 .rdi .rdx)
          (X86_64Instruction.step (pop_r64 .rdx) spike2Row2AfterExtraction))))
...
def spike2Row2AfterLineTerminator : X86_64MachineState :=
  X86_64Instruction.step (add_r64_imm8 .rdi 1)
    (X86_64Instruction.step (mov_mem8 .rdi .rax)
      (X86_64Instruction.step (mov_r64_imm64 .rax 10)
        (X86_64Instruction.step (add_r64_imm8 .rdi 1)
          (X86_64Instruction.step (mov_mem8 .rdi .rax)
            (X86_64Instruction.step (mov_r64_imm64 .rax 13) spike2Row2AfterWrite)))))

/-- Register setup immediately before the production Linux `SYS_write` instruction. -/
def spike2Row2BeforeWriteSyscall : X86_64MachineState :=
      ...
      (X86_64Instruction.step (mov_r64 .rdx .r8)
        (X86_64Instruction.step (sub_r64 .r8 .rsi)
          (X86_64Instruction.step (lea_rsp .rsi 0x40)
            (X86_64Instruction.step (mov_r64 .r8 .rdi) spike2Row2AfterLineTerminator)))))

/-- The host-resumed state after the real selected `SYS_write` call. -/
def spike2Row2AfterWriteSyscall : X86_64MachineState :=
  (sysWriteHook (Event := AnyEvent)
    (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall)).1

/-- Reverse event accumulator after row one's selected write boundary. -/
def spike2Row2WriteEventsRev : List AnyEvent :=
...
def spike2Row2AfterRecurrence : X86_64MachineState :=
      ...
      (X86_64Instruction.step (mov_r64 .r15 .r8)
        (X86_64Instruction.step (mov_r64 .r14 .r15)
          (X86_64Instruction.step (add_r64 .r8 .r15)
            (X86_64Instruction.step (mov_r64 .r8 .r14) spike2Row2AfterWriteSyscall)))))
...
theorem spike2_row2_index_header_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      (spike2AfterMainHeader spike2Row1AfterRecurrence) ([] : List AnyEvent)
      spike2Row2AfterIndexHeader [] [] := by
...
theorem spike2_row2_index_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 10
      spike2Row2AfterIndexHeader ([] : List AnyEvent) spike2Row2AfterIndex [] [] := by
...
theorem spike2_row2_value_setup_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 3
      spike2Row2AfterIndex ([] : List AnyEvent) spike2Row2AfterValueSetup [] [] := by
...
theorem spike2_row2_extraction_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 7
      spike2Row2AfterValueSetup ([] : List AnyEvent) spike2Row2AfterExtraction [] [] := by
  refine ProductionPrefix.SelectedPrefix.ordinary ({
...
theorem spike2_row2_write_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 5
      spike2Row2AfterExtraction ([] : List AnyEvent) spike2Row2AfterWrite [] [] := by
  refine ProductionPrefix.SelectedPrefix.ordinary ({
      encoding := .pop .rdx
      safeFallthrough := by intro _ _; rfl }) ?_ ?_ ?_ ?_ ?_
  · rfl
...
theorem spike2_row2_line_terminator_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row2AfterWrite ([] : List AnyEvent) spike2Row2AfterLineTerminator [] [] := by
...
theorem spike2_row2_write_setup_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row2AfterLineTerminator ([] : List AnyEvent) spike2Row2BeforeWriteSyscall [] [] := by
...
theorem spike2_row2_write_syscall_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 1
      spike2Row2BeforeWriteSyscall ([] : List AnyEvent) spike2Row2AfterWriteSyscall
      (accumulateEvent [] (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall)).2)
      (emittedBy (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall)).2) := by
  have hstep : X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall =
      { (spike2Row2BeforeWriteSyscall.setGpr64 .rcx
          (spike2Row2BeforeWriteSyscall.rip + 2)).setGpr64 .r11
          spike2Row2BeforeWriteSyscall.flags with rip := linuxSyscallEntry } := rfl
  have hrax : (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall).gprs .rax =
      SYS_write := by rfl
  have hstdout : (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall).gprs .rdi =
      1 := by rfl
  have hrip : (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall).rip =
      linuxSyscallEntry := by rw [hstep]
  have hsafe : (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall).fault = none := by
    rfl
  refine ProductionPrefix.SelectedPrefix.hostIntercept (Event := AnyEvent)
    ...
    (hooked := spike2Row2AfterWriteSyscall)
    (event := (sysWriteHook (Event := AnyEvent)
      (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall)).2) ?_ ?_ ?_ ?_ ?_
  · rfl
  · change selectedNonInputPlatformCall
      (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall).rip
      (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall) = true
    rw [selectedNonInputPlatformCall, hrip]
    simp [selectedNonInputLinuxCall, hrax, SYS_write, linuxSyscallEntry]
  · change (if (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall).rip ==
    ...
    rfl
  · unfold spike2Row2AfterWriteSyscall
    simp only [sysWriteHook, hstdout, ↓reduceIte]
    change (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall).fault = none
    exact hsafe
  · exact .nil _ _

/-- The real Fibonacci register update and linked `jmp near main_loop` complete row one. -/
theorem spike2_row2_recurrence_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row2AfterWriteSyscall spike2Row2WriteEventsRev spike2Row2AfterRecurrence
      spike2Row2WriteEventsRev [] := by
...
theorem spike2_row2_opening_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 18
      spike2Row1AfterRecurrence ([] : List AnyEvent) spike2Row2AfterIndex [] [] := by
  have header := spike2_main_header_selected_prefix 1 spike2Row1AfterRecurrence ([] : List AnyEvent)
    (by omega) rfl rfl rfl
  have opening := ProductionPrefix.SelectedPrefix.append header
    spike2_row2_index_header_selected_prefix
  have joined := ProductionPrefix.SelectedPrefix.append opening spike2_row2_index_selected_prefix
  simpa using joined
...
theorem spike2_row2_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 52
      spike2Row1AfterRecurrence ([] : List AnyEvent) spike2Row2AfterRecurrence spike2Row2WriteEventsRev
      (emittedBy (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row2BeforeWriteSyscall)).2) := by
  have opening := spike2_row2_opening_selected_prefix
  have valueSetup := ProductionPrefix.SelectedPrefix.append opening
    spike2_row2_value_setup_selected_prefix
  have extraction := ProductionPrefix.SelectedPrefix.append valueSetup
    spike2_row2_extraction_selected_prefix
  have write := ProductionPrefix.SelectedPrefix.append extraction spike2_row2_write_selected_prefix
  have terminator := ProductionPrefix.SelectedPrefix.append write
    spike2_row2_line_terminator_selected_prefix
  have writeSetup := ProductionPrefix.SelectedPrefix.append terminator
    spike2_row2_write_setup_selected_prefix
  have writeSyscall := ProductionPrefix.SelectedPrefix.append writeSetup
    spike2_row2_write_syscall_selected_prefix
  have recurrence := ProductionPrefix.SelectedPrefix.append writeSyscall
    spike2_row2_recurrence_selected_prefix
  simpa [spike2Row2WriteEventsRev] using recurrence
...
theorem spike2_row2_after_recurrence_boundary :
    ...
    spike2Row2AfterRecurrence.rsp = spike2Row1AfterRecurrence.rsp ∧
