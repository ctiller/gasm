...
def spike2Row4AfterIndexHeader : X86_64MachineState :=
        ...
        (X86_64Instruction.step (mov_rsp_byte 0x42 0x62)
          (X86_64Instruction.step (mov_rsp_byte 0x41 0x69)
            (X86_64Instruction.step (mov_rsp_byte 0x40 0x46)
              (spike2AfterMainHeader spike2Row3AfterRecurrence))))))

/-- The real joined value-format entry reached by the row-one one-digit branch. -/
def spike2Row4AfterIndex : X86_64MachineState :=
        ...
        (X86_64Instruction.step (push_r64 .rdx)
          (X86_64Instruction.step (add_r64_imm8 .rdx 0x30)
            (X86_64Instruction.step (div_r64 .r10)
              (X86_64Instruction.step (xor_r32 .edx .edx) spike2Row4AfterValueSetup))))))

/-- The terminating pop/write pass which materializes row one's sole decimal digit. -/
def spike2Row4AfterWrite : X86_64MachineState :=
    ...
    (X86_64Instruction.step (sub_r64_imm8 .rcx 1)
      (X86_64Instruction.step (add_r64_imm8 .rdi 1)
        (X86_64Instruction.step (mov_mem8 .rdi .rdx)
          (X86_64Instruction.step (pop_r64 .rdx) spike2Row4AfterExtraction))))
...
def spike2Row4AfterLineTerminator : X86_64MachineState :=
  X86_64Instruction.step (add_r64_imm8 .rdi 1)
    (X86_64Instruction.step (mov_mem8 .rdi .rax)
      (X86_64Instruction.step (mov_r64_imm64 .rax 10)
        (X86_64Instruction.step (add_r64_imm8 .rdi 1)
          (X86_64Instruction.step (mov_mem8 .rdi .rax)
            (X86_64Instruction.step (mov_r64_imm64 .rax 13) spike2Row4AfterWrite)))))

/-- Register setup immediately before the production Linux `SYS_write` instruction. -/
def spike2Row4BeforeWriteSyscall : X86_64MachineState :=
      ...
      (X86_64Instruction.step (mov_r64 .rdx .r8)
        (X86_64Instruction.step (sub_r64 .r8 .rsi)
          (X86_64Instruction.step (lea_rsp .rsi 0x40)
            (X86_64Instruction.step (mov_r64 .r8 .rdi) spike2Row4AfterLineTerminator)))))

/-- The host-resumed state after the real selected `SYS_write` call. -/
def spike2Row4AfterWriteSyscall : X86_64MachineState :=
  (sysWriteHook (Event := AnyEvent)
    (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall)).1

/-- Reverse event accumulator after row one's selected write boundary. -/
def spike2Row4WriteEventsRev : List AnyEvent :=
...
def spike2Row4AfterRecurrence : X86_64MachineState :=
      ...
      (X86_64Instruction.step (mov_r64 .r15 .r8)
        (X86_64Instruction.step (mov_r64 .r14 .r15)
          (X86_64Instruction.step (add_r64 .r8 .r15)
            (X86_64Instruction.step (mov_r64 .r8 .r14) spike2Row4AfterWriteSyscall)))))
...
theorem spike2_row4_index_header_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      (spike2AfterMainHeader spike2Row3AfterRecurrence) ([] : List AnyEvent)
      spike2Row4AfterIndexHeader [] [] := by
...
theorem spike2_row4_index_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 10
      spike2Row4AfterIndexHeader ([] : List AnyEvent) spike2Row4AfterIndex [] [] := by
...
theorem spike2_row4_value_setup_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 3
      spike2Row4AfterIndex ([] : List AnyEvent) spike2Row4AfterValueSetup [] [] := by
...
theorem spike2_row4_extraction_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 7
      spike2Row4AfterValueSetup ([] : List AnyEvent) spike2Row4AfterExtraction [] [] := by
  refine ProductionPrefix.SelectedPrefix.ordinary ({
...
theorem spike2_row4_write_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 5
      spike2Row4AfterExtraction ([] : List AnyEvent) spike2Row4AfterWrite [] [] := by
  refine ProductionPrefix.SelectedPrefix.ordinary ({
      encoding := .pop .rdx
      safeFallthrough := by intro _ _; rfl }) ?_ ?_ ?_ ?_ ?_
  · rfl
...
theorem spike2_row4_line_terminator_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row4AfterWrite ([] : List AnyEvent) spike2Row4AfterLineTerminator [] [] := by
...
theorem spike2_row4_write_setup_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row4AfterLineTerminator ([] : List AnyEvent) spike2Row4BeforeWriteSyscall [] [] := by
...
theorem spike2_row4_write_syscall_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 1
      spike2Row4BeforeWriteSyscall ([] : List AnyEvent) spike2Row4AfterWriteSyscall
      (accumulateEvent [] (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall)).2)
      (emittedBy (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall)).2) := by
  have hstep : X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall =
      { (spike2Row4BeforeWriteSyscall.setGpr64 .rcx
          (spike2Row4BeforeWriteSyscall.rip + 2)).setGpr64 .r11
          spike2Row4BeforeWriteSyscall.flags with rip := linuxSyscallEntry } := rfl
  have hrax : (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall).gprs .rax =
      SYS_write := by rfl
  have hstdout : (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall).gprs .rdi =
      1 := by rfl
  have hrip : (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall).rip =
      linuxSyscallEntry := by rw [hstep]
  have hsafe : (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall).fault = none := by
    rfl
  refine ProductionPrefix.SelectedPrefix.hostIntercept (Event := AnyEvent)
    ...
    (hooked := spike2Row4AfterWriteSyscall)
    (event := (sysWriteHook (Event := AnyEvent)
      (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall)).2) ?_ ?_ ?_ ?_ ?_
  · rfl
  · change selectedNonInputPlatformCall
      (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall).rip
      (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall) = true
    rw [selectedNonInputPlatformCall, hrip]
    simp [selectedNonInputLinuxCall, hrax, SYS_write, linuxSyscallEntry]
  · change (if (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall).rip ==
    ...
    rfl
  · unfold spike2Row4AfterWriteSyscall
    simp only [sysWriteHook, hstdout, ↓reduceIte]
    change (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall).fault = none
    exact hsafe
  · exact .nil _ _

/-- The real Fibonacci register update and linked `jmp near main_loop` complete row one. -/
theorem spike2_row4_recurrence_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row4AfterWriteSyscall spike2Row4WriteEventsRev spike2Row4AfterRecurrence
      spike2Row4WriteEventsRev [] := by
...
theorem spike2_row4_opening_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 18
      spike2Row3AfterRecurrence ([] : List AnyEvent) spike2Row4AfterIndex [] [] := by
  have header := spike2_main_header_selected_prefix 3 spike2Row3AfterRecurrence ([] : List AnyEvent)
    (by omega) rfl rfl rfl
  have opening := ProductionPrefix.SelectedPrefix.append header
    spike2_row4_index_header_selected_prefix
  have joined := ProductionPrefix.SelectedPrefix.append opening spike2_row4_index_selected_prefix
  simpa using joined
...
theorem spike2_row4_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 52
      spike2Row3AfterRecurrence ([] : List AnyEvent) spike2Row4AfterRecurrence spike2Row4WriteEventsRev
      (emittedBy (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row4BeforeWriteSyscall)).2) := by
  have opening := spike2_row4_opening_selected_prefix
  have valueSetup := ProductionPrefix.SelectedPrefix.append opening
    spike2_row4_value_setup_selected_prefix
  have extraction := ProductionPrefix.SelectedPrefix.append valueSetup
    spike2_row4_extraction_selected_prefix
  have write := ProductionPrefix.SelectedPrefix.append extraction spike2_row4_write_selected_prefix
  have terminator := ProductionPrefix.SelectedPrefix.append write
    spike2_row4_line_terminator_selected_prefix
  have writeSetup := ProductionPrefix.SelectedPrefix.append terminator
    spike2_row4_write_setup_selected_prefix
  have writeSyscall := ProductionPrefix.SelectedPrefix.append writeSetup
    spike2_row4_write_syscall_selected_prefix
  have recurrence := ProductionPrefix.SelectedPrefix.append writeSyscall
    spike2_row4_recurrence_selected_prefix
  simpa [spike2Row4WriteEventsRev] using recurrence
...
theorem spike2_row4_after_recurrence_boundary :
    ...
    spike2Row4AfterRecurrence.rsp = spike2Row3AfterRecurrence.rsp ∧
