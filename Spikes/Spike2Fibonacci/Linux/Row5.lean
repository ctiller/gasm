...
def spike2Row5AfterIndexHeader : X86_64MachineState :=
        ...
        (X86_64Instruction.step (mov_rsp_byte 0x42 0x62)
          (X86_64Instruction.step (mov_rsp_byte 0x41 0x69)
            (X86_64Instruction.step (mov_rsp_byte 0x40 0x46)
              (spike2AfterMainHeader spike2Row4AfterRecurrence))))))

/-- The real joined value-format entry reached by the row-one one-digit branch. -/
def spike2Row5AfterIndex : X86_64MachineState :=
        ...
        (X86_64Instruction.step (push_r64 .rdx)
          (X86_64Instruction.step (add_r64_imm8 .rdx 0x30)
            (X86_64Instruction.step (div_r64 .r10)
              (X86_64Instruction.step (xor_r32 .edx .edx) spike2Row5AfterValueSetup))))))

/-- The terminating pop/write pass which materializes row one's sole decimal digit. -/
def spike2Row5AfterWrite : X86_64MachineState :=
    ...
    (X86_64Instruction.step (sub_r64_imm8 .rcx 1)
      (X86_64Instruction.step (add_r64_imm8 .rdi 1)
        (X86_64Instruction.step (mov_mem8 .rdi .rdx)
          (X86_64Instruction.step (pop_r64 .rdx) spike2Row5AfterExtraction))))
...
def spike2Row5AfterLineTerminator : X86_64MachineState :=
  X86_64Instruction.step (add_r64_imm8 .rdi 1)
    (X86_64Instruction.step (mov_mem8 .rdi .rax)
      (X86_64Instruction.step (mov_r64_imm64 .rax 10)
        (X86_64Instruction.step (add_r64_imm8 .rdi 1)
          (X86_64Instruction.step (mov_mem8 .rdi .rax)
            (X86_64Instruction.step (mov_r64_imm64 .rax 13) spike2Row5AfterWrite)))))

/-- Register setup immediately before the production Linux `SYS_write` instruction. -/
def spike2Row5BeforeWriteSyscall : X86_64MachineState :=
      ...
      (X86_64Instruction.step (mov_r64 .rdx .r8)
        (X86_64Instruction.step (sub_r64 .r8 .rsi)
          (X86_64Instruction.step (lea_rsp .rsi 0x40)
            (X86_64Instruction.step (mov_r64 .r8 .rdi) spike2Row5AfterLineTerminator)))))

/-- The host-resumed state after the real selected `SYS_write` call. -/
def spike2Row5AfterWriteSyscall : X86_64MachineState :=
  (sysWriteHook (Event := AnyEvent)
    (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall)).1

/-- Reverse event accumulator after row one's selected write boundary. -/
def spike2Row5WriteEventsRev : List AnyEvent :=
...
def spike2Row5AfterRecurrence : X86_64MachineState :=
      ...
      (X86_64Instruction.step (mov_r64 .r15 .r8)
        (X86_64Instruction.step (mov_r64 .r14 .r15)
          (X86_64Instruction.step (add_r64 .r8 .r15)
            (X86_64Instruction.step (mov_r64 .r8 .r14) spike2Row5AfterWriteSyscall)))))
...
theorem spike2_row5_index_header_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      (spike2AfterMainHeader spike2Row4AfterRecurrence) ([] : List AnyEvent)
      spike2Row5AfterIndexHeader [] [] := by
...
theorem spike2_row5_index_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 10
      spike2Row5AfterIndexHeader ([] : List AnyEvent) spike2Row5AfterIndex [] [] := by
...
theorem spike2_row5_value_setup_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 3
      spike2Row5AfterIndex ([] : List AnyEvent) spike2Row5AfterValueSetup [] [] := by
...
theorem spike2_row5_extraction_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 7
      spike2Row5AfterValueSetup ([] : List AnyEvent) spike2Row5AfterExtraction [] [] := by
  refine ProductionPrefix.SelectedPrefix.ordinary ({
...
theorem spike2_row5_write_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 5
      spike2Row5AfterExtraction ([] : List AnyEvent) spike2Row5AfterWrite [] [] := by
  refine ProductionPrefix.SelectedPrefix.ordinary ({
      encoding := .pop .rdx
      safeFallthrough := by intro _ _; rfl }) ?_ ?_ ?_ ?_ ?_
  · rfl
...
theorem spike2_row5_line_terminator_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row5AfterWrite ([] : List AnyEvent) spike2Row5AfterLineTerminator [] [] := by
...
theorem spike2_row5_write_setup_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row5AfterLineTerminator ([] : List AnyEvent) spike2Row5BeforeWriteSyscall [] [] := by
...
theorem spike2_row5_write_syscall_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 1
      spike2Row5BeforeWriteSyscall ([] : List AnyEvent) spike2Row5AfterWriteSyscall
      (accumulateEvent [] (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall)).2)
      (emittedBy (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall)).2) := by
  have hstep : X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall =
      { (spike2Row5BeforeWriteSyscall.setGpr64 .rcx
          (spike2Row5BeforeWriteSyscall.rip + 2)).setGpr64 .r11
          spike2Row5BeforeWriteSyscall.flags with rip := linuxSyscallEntry } := rfl
  have hrax : (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall).gprs .rax =
      SYS_write := by rfl
  have hstdout : (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall).gprs .rdi =
      1 := by rfl
  have hrip : (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall).rip =
      linuxSyscallEntry := by rw [hstep]
  have hsafe : (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall).fault = none := by
    rfl
  refine ProductionPrefix.SelectedPrefix.hostIntercept (Event := AnyEvent)
    ...
    (hooked := spike2Row5AfterWriteSyscall)
    (event := (sysWriteHook (Event := AnyEvent)
      (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall)).2) ?_ ?_ ?_ ?_ ?_
  · rfl
  · change selectedNonInputPlatformCall
      (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall).rip
      (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall) = true
    rw [selectedNonInputPlatformCall, hrip]
    simp [selectedNonInputLinuxCall, hrax, SYS_write, linuxSyscallEntry]
  · change (if (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall).rip ==
    ...
    rfl
  · unfold spike2Row5AfterWriteSyscall
    simp only [sysWriteHook, hstdout, ↓reduceIte]
    change (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall).fault = none
    exact hsafe
  · exact .nil _ _

/-- The real Fibonacci register update and linked `jmp near main_loop` complete row one. -/
theorem spike2_row5_recurrence_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 6
      spike2Row5AfterWriteSyscall spike2Row5WriteEventsRev spike2Row5AfterRecurrence
      spike2Row5WriteEventsRev [] := by
...
theorem spike2_row5_opening_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 18
      spike2Row4AfterRecurrence ([] : List AnyEvent) spike2Row5AfterIndex [] [] := by
  have header := spike2_main_header_selected_prefix 4 spike2Row4AfterRecurrence ([] : List AnyEvent)
    (by omega) rfl rfl rfl
  have opening := ProductionPrefix.SelectedPrefix.append header
    spike2_row5_index_header_selected_prefix
  have joined := ProductionPrefix.SelectedPrefix.append opening spike2_row5_index_selected_prefix
  simpa using joined
...
theorem spike2_row5_selected_prefix :
    ProductionPrefix.SelectedPrefix selectedNonInputPlatformCall spike2Indexed 52
      spike2Row4AfterRecurrence ([] : List AnyEvent) spike2Row5AfterRecurrence spike2Row5WriteEventsRev
      (emittedBy (sysWriteHook (Event := AnyEvent)
        (X86_64Instruction.step syscall_op spike2Row5BeforeWriteSyscall)).2) := by
  have opening := spike2_row5_opening_selected_prefix
  have valueSetup := ProductionPrefix.SelectedPrefix.append opening
    spike2_row5_value_setup_selected_prefix
  have extraction := ProductionPrefix.SelectedPrefix.append valueSetup
    spike2_row5_extraction_selected_prefix
  have write := ProductionPrefix.SelectedPrefix.append extraction spike2_row5_write_selected_prefix
  have terminator := ProductionPrefix.SelectedPrefix.append write
    spike2_row5_line_terminator_selected_prefix
  have writeSetup := ProductionPrefix.SelectedPrefix.append terminator
    spike2_row5_write_setup_selected_prefix
  have writeSyscall := ProductionPrefix.SelectedPrefix.append writeSetup
    spike2_row5_write_syscall_selected_prefix
  have recurrence := ProductionPrefix.SelectedPrefix.append writeSyscall
    spike2_row5_recurrence_selected_prefix
  simpa [spike2Row5WriteEventsRev] using recurrence
...
theorem spike2_row5_after_recurrence_boundary :
    spike2Row5AfterRecurrence.rip = spike2MainLoopRip ∧
    spike2Row5AfterRecurrence.gprs .r13 = 6 ∧
    spike2Row5AfterRecurrence.gprs .r14 = 8 ∧
    spike2Row5AfterRecurrence.gprs .r15 = 13 ∧
    spike2Row5AfterRecurrence.rsp = spike2Row4AfterRecurrence.rsp ∧
    spike2Row5AfterRecurrence.fault = none := by
  decide
