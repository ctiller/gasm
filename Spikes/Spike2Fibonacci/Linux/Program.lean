...
/- REF: docs/SPIKES.md#3-spike-progression-roadmap -/
/-- Symbolic main driver program for Linux Spike 2:
    ...
    division/modulo itoa instructions, and outputting to standard output via Linux sys_write. -/
def spike2SymbolicProgram : List SymbolicInstr := [
  -- 1. Allocate 136-byte stack frame (maintaining 16-byte alignment)
  instr (sub_rsp32 136),

  -- 2. Initialize dynamic Fibonacci registers: i = 1, a = 1, b = 1
  instr (mov_r64_imm64 .r13 1),
  instr (mov_r64_imm64 .r14 1),
  instr (mov_r64_imm64 .r15 1),

  -- 3. Main loop header: for i = 1 to 90
  label "main_loop",
  instr (cmp_r64_imm8 .r13 91),
  jge_near_label "exit_loop",

  -- 4. Step 1: Write "Fib(" directly into stack buffer [rsp + 0x40]
  instr (mov_rsp_byte 0x40 0x46),
  instr (mov_rsp_byte 0x41 0x69),
  instr (mov_rsp_byte 0x42 0x62),
  instr (mov_rsp_byte 0x43 0x28),

  -- 5. Step 2: Format loop index i (in r13) into stack buffer via hardware division
  ...
  -- 6. Step 3: Hardware itoa on 64-bit Fibonacci value in r14
  label "itoa_start",
  ...
  -- 8. Step 5: Output line to console via Linux sys_write (1, rsp+0x40, len)
  instr (mov_r64 .r8 .rdi),
  instr (lea_rsp .rsi 0x40),
  instr (sub_r64 .r8 .rsi),
  instr (mov_r64 .rdx .r8),
  instr (mov_r32 .edi 1),
  instr (mov_r32 .eax 1),
  instr syscall_op,

  -- 9. Step 6: Advance Fibonacci recurrence in hardware registers
  ...
  -- 10. Exit cleanly with code 0 via sys_exit(0)
  label "exit_loop",
...
/- REF: docs/SPIKES.md#3-spike-progression-roadmap -/
/-- Linked binary program artifact for Linux Spike 2. -/
def spike2Linked : LinkedLinuxProgram :=
  linkLinuxProgramStatic spike2SymbolicProgram []

/- REF: docs/SPIKES.md#3-spike-progression-roadmap -/
/-- Lowered concrete machine instruction sequence for Linux Spike 2. -/
def spike2Instructions : List X86_64Instr :=
  spike2Linked.instructions

/- REF: docs/SPIKES.md#3-spike-progression-roadmap -/
/-- Complete Linux executable specification for Spike 2. -/
def spike2Executable : LinuxExecutable :=
