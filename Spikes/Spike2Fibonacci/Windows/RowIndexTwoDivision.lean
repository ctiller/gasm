...
import Spikes.Spike2Fibonacci.Windows.RowDivisionFacts

namespace Spikes.Spike2Fibonacci.Windows

local instance (priority := 1100) spike2WindowsRuntimeForRowIndexTwoDivision :
...
structure Spike2DivisionResult (initial : X86_64MachineState)
    (eventsRev : List AnyEvent)
    extends Spike2MemoryPreservingSliceResult initial eventsRev 4 5368713362 where
  quotient : final.gprs .rax = UInt64.ofNat ((initial.gprs .r13).toNat / 10)
  remainder : final.gprs .rdx = UInt64.ofNat ((initial.gprs .r13).toNat % 10)
...
opaque spike2_two_digit_division_slice (state : X86_64MachineState)
    ...
    Spike2DivisionResult state eventsRev := by
  ...
  exact {
    ...
    registers := frame
    fibRegisters := spike2_two_digit_division_fibRegisterFrame state
    rip := finalRip
    ...
    memory := by
      ...
      rw [step_div_r64_by10 s3 zeroHigh divisor]
      rfl
    quotient := spike2_two_digit_division_quotient state
    remainder := spike2_two_digit_division_remainder state }
