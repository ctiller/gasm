...
theorem spike1_outcome_external_input_frame (environment : Environment) :
  ...
  apply runProgramOutcomeLoop_external_input_frame
    ...
    (indexInstructions spike1Executable.load.rip spike1Instructions) (allowHalted := true)
  · intro instr _
...
def spike1VerifiedProgram :
  ...
  importsCovered   := by
    ...
    contradiction
  capabilitiesConnection := by trivial
  entryContext     := fun _ => ()
  entryEstablished := by
    intro environment
    trivial
  platformAdmissible := by
    ...
    exact selectedExecutionTerminates_isAdmissible true selectedNonInputPlatformCall
      (indexInstructions spike1Executable.load.rip spike1Instructions) 50000
