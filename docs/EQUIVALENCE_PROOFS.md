...
- **Concurrency generalizes the pair, it does not replace it.** Once M3/M6-T threading lands, a typical program contains *several* reactive loops in one address space. The contract then becomes: one inner/outer pair **per loop** (each loop's inner obligation is deterministic both-ways equality for its iteration; each loop's outer obligation is its own progress), plus **composition obligations** across loops — absence of deadlock/livelock at the declared synchronization points, and any fairness assumptions stated explicitly as part of the contract rather than assumed. The per-loop pairs must remain independently checkable (a loop's inner proof must not depend on another loop's scheduling). Cross-loop interaction uses the profile-selected labelled causal paths and trace projection defined in `docs/MEMORY_MODEL.md` §11; target execution consistency remains a separate proof layer. Multiprocessing is a further, independently selected M6-PL/M6-PW composition: global events are qualified by generative `ProcessInstanceId`, cross-process data/result transfer uses explicit IPC/shared-memory/object channels, process status observation and reaping are not task `JoinRight`, and resource survival follows declared failure domains rather than an implicit global stop.
