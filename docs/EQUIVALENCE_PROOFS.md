...
**1. Observational direction and anti-vacuity.**
- For a **closed deterministic specialization**, trace equality remains the result, but its relational derivation needs the complete premises: exact closed environment/provider transcript, singleton target execution, functional observation/refinement, and a forward witness for the exact root-required observation. Singleton execution plus backward soundness proves permission only, not equality.
- For **nondeterministic specifications**, obligations split four ways: exact safety/classification for every finite prefix; refinement of every terminal machine execution to a permitted source result; absence or root-declared disposition of stuck/fault states; and separately named required-behavior/progress witnesses under exact eligibility, provider and fairness premises. An infinite/maximal execution refines a source trace only when the root owns an infinite observation; otherwise termination/no-infinite-run is a separate conditional theorem. Universal provider soundness covers every concrete transition. Conditional coverage applies only to root-required or selected eligible result classes; it does not demand every theoretically permitted behavior in every state.
- **Non-terminating programs (reactive loops) are enforced as inner/outer proof packages.** A program whose spec is an infinite service loop (e.g. a server) carries a distinct contract type (`VerifiedReactiveProgram` — ratified direction, not yet implemented; Spike 4 currently ships under the old plain `VerifiedProgram` and is a migration target). The **inner** package is the applicable per-iteration result: universal transition/prefix safety, terminal/result refinement, stuck/fault disposition, and eligible required-behavior witnesses. Closed deterministic iterations recover equality only through the specialization above. The **outer** package proves progress only under exact arrival eligibility, provider availability/results, cancellation, resource and fairness premises; it never claims unconditionally that every arrival is consumed or the loop cannot wedge.
- **Concurrency composes the packages; it does not force deterministic equality.** Once M3/M6-T threading lands, a typical program contains several reactive loops in one root host process; independently selected GPU/device loops may inhabit different target-indexed address/reference domains. Each loop retains its independently checkable inner result and explicitly premised outer progress claim, plus composition obligations across loops such as the selected deadlock/livelock property. Cross-loop interaction uses the profile-selected labelled causal paths and trace projection defined in `docs/MEMORY_MODEL.md` §11; target execution consistency remains separate. Multiprocessing is deferred beyond M9 under `docs/FUTURE_PROCESS_MODEL.md` and adds no current equivalence obligation.
...
> The durable concurrency design and its safety/liveness split are now in
> `docs/MEMORY_MODEL.md` §§8 and 12.
...
The split is about reusable routine facts, not a complete external-call certificate. Consumer 2 also
needs M1's relational caller/link witness establishing the exact logical arguments, binding, live
world and precondition; a concrete target then proves its M2-B ABI/admissibility and artifact link.
Fresh logical result identities and authority stay in the relational after-world rather than being
decoded from the physical `m_final`. Thus `memcpy_callability` below is one reusable component, not
the whole caller obligation.
