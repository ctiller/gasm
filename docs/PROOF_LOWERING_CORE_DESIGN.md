...
migration, or integration. Craig owns semantic and architecture decisions; `trustplan` owns this
design cycle. Any later implementation or integration requires separate owner authorization.
...
The source contract is not defined by its interpreter: its semantics and proofs do not import or
definitionally depend on target code. A substitution/countermodel review should be able to vary the
target without changing source meaning. This independence test does not require constructing a
...
- Downward demands are predicates from the root or a domain contract over the exact selected
  definitions, executions and results. Only the selected demanded properties and prerequisites
  needed to derive them flow toward leaves. Stronger mechanically derived facts may remain private
  or reusable, but impose no consumer or leaf burden.
...
- route: preserve, directional translate, named delegate/transfer, proved discharge, or explicit
  nonterminal retention by the exact live continuation/post-state; and
...
the source does not observe it; its final owner still validates it. Retention is not terminal
disposition and cannot close a root merely because coverage is complete. At terminal closure every
applicable obligation is discharged, directionally translated, or transferred to a named live
owner/sink whose lifetime and cleanup remain tracked, except where the exact owner/root contract
explicitly permits that residual outcome.
...
For every fallback assembly consumer not expected to meet roughly ten lines of genuinely
program-specific proof per authored assembly line, perform exactly one human
specification/ownership/lowering feedback iteration. Decompose burden into source semantics,
reusable structural proof, semantic/domain transport, target/ABI leaves and the irreducible local
residual; identify the proper owner and smallest candidate theorem. The review may conclude that no
change is justified and direct proof remains. The ratio has no acceptance effect and creates no
repeat-until-convergence process. Do not game the denominator by generated assembly,
moved/relabelled lines, golfing or weakening the claim. A shared implementation still requires
Craig's approval and a materially different second consumer; otherwise keep it local.
...
is an architectural signal that triggers the one human review above. Novel implementations may
carry higher burden proportional to their deviation without compatibility bias toward the reference
lowering.

Proposal review authorizes only a later owner-approved experiment. An implemented interface shape is
not validated for current use until two universal `VerifiedProgram` consumers—the
checked-access pathfinder and Spike 1—exercise it. The seven proof-architecture consumers are a
separate cross-domain pressure matrix. This may stabilize a disposable implementation shape; it
does not freeze datatypes/APIs or promote them into compatibility-preserved semantic contracts.
...
concrete core interface be proposed for use. Its shape remains disposable unless Craig explicitly
promotes a guarantee into an owner-level semantic contract.

## 15. Historical and documentation disposition

Earlier obligation/capability-lowering drafts, prior dossier reviews, and all current proof facades
are historical evidence. They do not authorize compatibility work. This document is the candidate core design;
`MEMORY_MODEL_PRESERVATION_INVENTORY.md` is an independent pass-one baseline; the current
`PROPOSED_PROOF_ARCHITECTURE.md` pressure-tests all seven consumers.
...
4. Validate two universal roots and mutation controls.
5. Only then propose a disposable concrete interface for current use and reconcile surrounding docs.
