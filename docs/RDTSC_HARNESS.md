# RDTSC/RDTSCP Hardware Measurement Harness — Contract and Evidence

> Current contract for `Gasm/Targets/X86_64/HardwareTimingHarness.lean` (containment and the
> raw-byte timing escape hatch) and `Gasm/Targets/X86_64/PerfHardwareFuzzer.lean` (generation,
> controls, containment/rank-order checking, and provisional calibration-file emission). Read
> alongside `docs/CALIBRATION_GOVERNANCE.md`, `docs/X86_ISA_EXPANSION_PREREQUISITES.md` P5,
> and `docs/VISION.md` §5. The measurement infrastructure exists; the mechanical calibration
> gate and accepted per-instruction coefficient bindings do not.

## 1. Why this exists

`Gasm/Targets/X86_64/Fuzzer.lean`'s `verifyPerfInvariants` checks uop conservation and
`0 < min ≤ nominal ≤ max` — both hold for an arbitrarily wrong model. The tree now contains an
RDTSC/RDTSCP measurement path in `HardwareTimingHarness.lean`, and
`PerfHardwareFuzzer.lean` can produce provisional files under `calibration/x86_64/`. Every
registered instruction's `costProvenance` nevertheless remains
`.modelInternalUnvalidated` (0 of N registered instances are `.cited`, per
`Tools/CheckX86Obligations.lean`'s summary) because no accepted calibration result is bound to
an instruction instance. `docs/CALIBRATION_GOVERNANCE.md` §9 rules out Agner Fog/uops.info as a
coefficient source (cross-check only), so an accepted self-measurement is still the remaining
path. The existence of a harness or a provisional JSON file is not, by itself, grounds to flip
`costProvenance` to `.cited`; §8 states the conservative promotion rule.

## 2. The wsc lesson, and what this design does about each part of it

The historical planning analysis found that wsc's own hardware comparison was a **manual, standalone**
...
wired into the build — is what the current harness closes. Concretely:
...
Why not model them as real `X86_64Instruction` instances:
...
- `CPUID` branches on `EAX`/`ECX` subleaf and returns model-specific data (vendor string,
  ...
  project's ISA model does not otherwise carry (there are no modeled MSRs or CPUID leaves).
...
- `verifyTimingOracleControls` (mirroring `verifyHardwareOracleControls`) runs immediately after
  the single batch is measured and BEFORE any result is trusted or written. A mandatory-control
  failure makes `runHardwareFuzz` return nonzero. The first two controls are unconditional; the
  third is required whenever an honestly contained base case exists and is otherwise reported as
  inconclusive rather than fabricated:
  1. **Positive control** — `nop_loop`'s timer-overhead-subtracted bracket total must land inside
     the inclusive band `[1, 60 × kernelUnrollPerRep]`, currently `[1, 30,720]` cycles for a
     512-copy bracket. Expressing the band at bracket scale preserves the generous 60-cycle
     per-instance ceiling without imposing a false ≥1-cycle throughput floor on each NOP. Zero
     is rejected explicitly and by the compile-checked `positiveControlRejectsZero` negative
     control.
  2. **Discriminating pair** (`docs/CALIBRATION_GOVERNANCE.md` §4 check 4 and §8) —
     `long_dependent_chain` (512 serially-dependent `ADD reg, 1` instructions per bracket) must
     measure at least
  ...
  3. **Mis-calibration negative control** — deliberately subtracting a **wrong** (10x inflated)
     ...
     is an explicit harness acceptance item: a deliberately mis-calibrated timer-overhead
     constant must be demonstrated to break containment. It runs against
     whichever kernel in the full suite honestly contains first (not hardcoded to one named
     kernel — §9's evidence run shows containment does not hold for a fixed kernel on every
     run); if **no** kernel honestly contains (§9: this happened on the initial evidence
     ...
     unconditionally; this control is a supplementary harness-specific acceptance criterion
     layered on top of it, not a replacement for it.
- No PMU and no PMU-derived uop counts: `docs/CALIBRATION_GOVERNANCE.md` §11 treats uop counts as
  internal model parameters rather than observables. The observable this harness produces is
  cycles, full stop.

## 5. Result-record extension

The timing path must carry raw cycle counts alongside the correctness harness's existing
GPR/RFLAGS/faulted observations. Interleaving a
...
the existing, working correctness oracle) and instead uses a **purpose-built result shape** in
`HardwareTimingHarness.lean`:
...
This preserves the required evidence in substance — a second,
...
warm-up and each measured bracket as straight-line code, generated host-side in Lean**, exactly the way
...
range — raised from an initial 5,000 during hardware validation: at 5,000 the
turbo/frequency ramp had not reliably stabilized by the time the measured section began, see §9).
The discarded/retained two-pass wrapper described next uses a stack-resident count of two and a
single backward branch *between* complete passes, outside every timestamped bracket; it reserves
no GPR and puts no loop-control instruction inside the measured stream.
...
one `measuredRepetitions`-bracket sequence and wraps it in a stack-counted two-pass loop. The
backward branch returns to the same instruction addresses; the first pass's results are simply
overwritten by the second, so only timings taken after those exact code pages were touched survive
to the output buffer. This reduced but did not fully
eliminate this machine's measurement dispersion (§9); it is recorded here as an implementation
...
calls the real `Fuzzer.generateRandomInstruction` (reusing the canonical generator rather than
duplicating it) and re-draws (bounded retries, falling back to a fixed
...
<per-bracket setup>     ; harness context, e.g. mov cl, 5; excluded from modeled cost
lfence                   ; setup completes before the timestamp
...
The opening `CPUID` must precede `perBracketSetupBytes` because it clobbers `RCX`; the following
`LFENCE` keeps that setup before `RDTSC`. Consequently `shl_by_cl` times exactly 512 copies of
`SHL RAX, CL`, while its harness-only `MOV CL, 5` is neither observed nor modeled. The empty
`timer_overhead` kernel uses the same bracket infrastructure.
...
explicitly labeled as **derived/amortized** and reporting-only, not a bare single-instruction
bracket reading. Containment instead compares the integer, overhead-subtracted bracket median
with model bounds for the identically replicated 512-copy uop stream, keeping both sides in
cycles per bracket and avoiding a rounded `Float` comparison. A
...
a mean is dominated by SMI/interrupt-tail outliers and a single median-of-63 is already robust to
a handful of spikes. `min`/`max` across the 63 repetitions are also recorded (so a coefficient
citation carries a distribution, not a point estimate). Following
`docs/CALIBRATION_GOVERNANCE.md` §2.3, `check_calibration.py`'s future dispersion guard
(IQR/median ratio bound, first-half-vs-second-half median drift bound) is **not implemented**
(`check_calibration.py` does not yet exist), but this harness's calibration JSON already stores
`raw_samples_cycles_unadjusted` in run order specifically so that guard can be added later without
a schema change, and the initial evidence report states each kernel's IQR/median ratio by hand
as the evidentiary substitute until that gate exists.

## 7. Where this harness is, and is not, valid to run

The ordinary `lake exe perf_fuzzer` command is a deterministic model-invariant check: it samples
no host timing and is valid on hosted CI. The explicit `--hardware` mode is different. A shared
or virtualized hosted vCPU does not give a trustworthy cycle-count signal (noisy neighbors,
masked/virtualized microarchitecture, no reliable low-level counters), so
`lake exe perf_fuzzer -- --hardware` **must never be added to hosted CI**. It requires a local or
self-hosted runner with a recorded, known CPU profile. The current emitter produces a Windows
x86-64 PE executable and calls Win32 APIs; native Linux/macOS and non-x86 hosts are not supported
by this implementation. The hardware mode is valid to run:

- On a physical, non-virtualized Windows x86-64 development machine with a stable, known
  microarchitecture, ideally on AC power with a "High performance"/no-throttle power plan.
- Interactively, by hand, the way the initial completion evidence was produced (§9).
...
- **A machine under concurrent heavy load from another process — not hypothetical; the initial
  completion evidence run hit exactly this (§9 gives the qualitative finding; the actual
...
The emitted executable requests a process-wide mask selecting logical processor 0 through
`SetProcessAffinityMask` (this is process affinity, not a `SetThreadAffinityMask` call). It does
not currently capture the API's BOOL result, so generated provenance records the request as
**unverified** rather than claiming confirmed pinning. When the request succeeds it removes OS
core migration as a variable; it does **not** detect or wait out *other* processes' load on that
...
Measurement and coefficient correction are deliberately separate. The harness does not edit any
coefficient's numeric value in `Uop.lean` or any `toUops` implementation. It can emit provisional
calibration artifacts and evaluate a narrow promotion criterion per instance, but it cannot bind
those results or claim that the underlying uop breakdown is correct:

> An instance's `costProvenance` may move from `.modelInternalUnvalidated` to
> `.cited "calibration/x86_64/<file>.json#reduction"` **only if** the timer-overhead-subtracted
> `perBracketMedian` falls within `[minCycles, maxCycles]` computed by `computeCycleBounds` over
> `kernelUnrollPerRep` copies of that instruction's `toUops`. The emitted instruction stream, the
> modeled uop stream, and the observed/model bounds are therefore all at the same scale: cycles
> per 512-copy bracket. `cyclesPerInstance` is a divided reporting field and is not the
> containment operand. Containment means the current bound was not falsified, but it is only a
> **necessary** promotion condition: the artifact must also pass the calibration-governance gate,
> be accepted, and be mechanically bound to the coefficient. This is deliberately weaker than "the
> uop count is correct" (which `docs/CALIBRATION_GOVERNANCE.md` §11 reserves for a
> later discriminating-kernel-pair promotion) and is stated as exactly that: `.cited` here
...
- `long_dependent_chain` — discrimination-pair partner (`AddR64Imm8`: 512 serially-dependent
  `ADD RAX, 1`).
- `shl_by_cl` (`ShlR64Cl`) — a named cost-model spot-check (modeled 1 uop; Intel P-cores are
...
non-numeric shape of the initial completion run; read the JSON files themselves for the
actual figures.

- **Control vectors**: the positive control and the discrimination pair (§4) both passed on the
  recorded completion run, and passed on every run attempted during development — the harness's
  *relative*/directional judgment (is kernel A reliably slower than kernel B) was consistently
  trustworthy.
- **Containment**: on the recorded completion run, containment (§8) did **not** hold for any
...
- **Producer-correction boundary**: the checked-in provisional JSON files predate four producer
  corrections: the positive-control unit, affinity provenance, same-address two-pass execution,
  and exclusion of per-bracket setup from the timed/model-comparison unit. They record a
  per-bracket observed value against a per-instance-shaped `[0, 60]` band, incorrectly say
  affinity was unset, describe disjoint duplicated code as same-address preconditioning, and (for
  `shl_by_cl`) include `MOV CL, 5` in the observation but not the modeled stream. They are therefore
  invalid as promotion evidence and must be regenerated by the corrected producer; no accepted
  `costProvenance` citation depends on them.
- **What this run still accomplishes**: a full set of real calibration JSON/`.md` file pairs
  ...
  provenance block, and a real controls block — evidence that the pipeline (harness → measurement
  → reduction → containment check → calibration file) operated end-to-end against genuine
  silicon. Re-running `lake exe perf_fuzzer -- --hardware` on a suitable unloaded machine makes a
  result ready for governance assessment; it does not guarantee citation, because the measurement
  may still falsify the model bounds and the artifact must still pass the future gate and binding
  review. Regeneration overwrites the provisional files in place, following Law 14's
  regenerable-not-transcribed discipline.

## 10. Rank-order tracking

Rank-order agreement is tracked across every pair of kernels in §9's hardware-timed suite that
has a modeled instruction: the fraction of pairs where the replicated model's `nominalCycles`
ordering agrees with the real per-bracket-median ordering is computed and printed
(`rank-order agreement: k/n pairs`). Every kernel uses the same 512-copy scale, so this is also
the ordering of the divided per-instance estimates. The metric is surfaced rather than gated, matching the harness's
explicit non-gating scope for it ("not necessarily gated pass/fail on day one, but measured and
surfaced"). Per `docs/REVIEW.md` §4.4/Law 14, the actual agreement fraction from any specific run is not repeated
...
preserve relative ordering either) — this is the metric later calibration work is meant to
improve, not a metric the current harness claims to have already fixed. A broader, model-only-evaluated
...
## 11. Vacuity-floor composition

`--hardware`'s entry point is subject to the same floor `PerfFuzzerCLI`'s existing `--count 0`
path already enforces: zero kernels measured is a hard failure, never a printed "100% SUCCESS," and the
control-vector failure path (§4) aborts before any measured result is trusted or written if the
...
- `DIV r64`, the second named cost-model spot-check, is **not** measured by this landing: `DIV`
  ...
  fuzz states) — named here as explicit follow-up calibration work, not a silent
  gap.
- Hypervisor-presence detection (§7) is not implemented — a documented human judgment call today.
- `check_calibration.py` does not exist yet; every calibration JSON the harness writes is
  therefore **provisional** — schema-conformant per `docs/CALIBRATION_GOVERNANCE.md` to the best
  of the implementation's ability to anticipate it, but not yet mechanically gated. Each file's own `.md`
  stub says so explicitly.
- Native timing-harness emitters/runners for Linux, macOS, and non-x86 hosts do not exist; the
  current `--hardware` implementation is Windows x86-64 PE-only (§7).
