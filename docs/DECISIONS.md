...
- **Jumps and calls are equally typed proof boundaries**: every reachable jump establishes its
  ...
  preservation; whole-path replay is neither required nor accepted as the architecture. Connecting
  that semantic CFG certificate to the exact final artifact remains mandatory applicability work.
