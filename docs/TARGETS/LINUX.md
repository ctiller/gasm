...
the `Stdlib/Data/ByteArray.lean` `.get!`/`getElem` bridge applied with Lean core's append/extract
laws across the parser's indices, plus the `WellFormed` predicate itself. **Status**: tracked as
follow-on work, not attempted in this pass; the empirical parser-stability fuzzer above is this
pass's substitute evidence, per this project's own established PNG-parser precedent
(`Stdlib/Png/StabilityFuzzer.lean`'s own header
