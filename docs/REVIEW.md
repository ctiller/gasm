...
1. **Type & Proof Integrity:** `lake build` must pass with zero `sorry` and zero un-authorized axioms.
   - *Liveness:* Verified routines must provide existential liveness (`progressProof`) preventing instant-termination vacuity.
...
7. **Differential Fuzzers (PR-scoped):** `lake exe test_roundtrip`, `lake exe x86_fuzzer` (hardware oracle), `lake exe encoding_fuzzer` (NASM oracle), `lake exe wasm_fuzzer` (node oracle), and `lake exe gzip_fuzzer` (python stdlib oracle) must all exit 0.
8. **Spike/Stdlib CLI test suites:** the spike and stdlib `test_*` executables that `defaultTargets` builds — `lake build` compiling them is not the same as running them (the same distinction item 4 draws for `check_gates_axioms`) — must also be invoked and must exit 0. See `scripts/run_gates.py`'s gate table for the current list.
