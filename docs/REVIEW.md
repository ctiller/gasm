...
- **Rungs 3 and 4 are forbidden debt; rung 2 is kernel proof**: a bare `decide` introduces no axiom, so the kernel itself checks it. Existing `native_decide`/`bv_decide` dependencies are exposed directly as hard failures. There is no ledger or entry mechanism: every occurrence must be replaced by a kernel/constructive proof or by a separately ratified proof-producing finite-domain mechanism before it can land.
