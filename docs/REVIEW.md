...
> **A verification obligation may be discharged by `native_decide` if and only if the proposition is universally quantified over its entire, finite domain. `native_decide` on a single ground instance — or on any proposition whose input domain is infinite or merely sampled — can NEVER satisfy a verification obligation.**

- **Rationale**: Implementations are generated by untrusted, creative processes ([`docs/VISION.md`](VISION.md#2-the-consequence-the-validation-gate-is-the-product)). A gate that examines sampled points is adversarially unsound: an implementation specialized to those points passes it while being wrong everywhere else.
- **Exhaustive finite-∀ is acceptable**: propositions quantified over a complete finite domain (all 256 byte values, all 16×16 register pairs, every constructor of an input enum) may be discharged by `decide` or `native_decide`; for exactly these exhaustive checks the compiled evaluator is admitted into the trusted base.
- **The finite domain must be the specification's actual input domain** (or an exhaustive cover of it) — exhaustiveness over a purpose-built stand-in (a single-constructor enum, `Unit`, a shrunken index type) is domain-shrinking under Law 9 and remains prohibited; Law 10 never licenses what Law 9 forbids. The ~25 existing spike contracts of this shape are grandfathered migration backlog (PA8), not compliant instances.
- **Single-instance ground checks are regression tests, not verification**: they MUST carry the `*_inst` suffix (Law 8), MUST live in designated test/regression modules rather than on `Spec.lean`/`Equivalence.lean` proof surfaces, and MUST NOT be cited — in names, docs, or review artifacts — as evidence that anything is verified.
- **Infinite-domain claims require structural proof**: ∀ over `ByteArray`, `Nat`, or arbitrary machine states is discharged only by kernel-checked structural proof (induction, simulation arguments, composition of routine contracts per [`docs/VISION.md`](VISION.md#4-tractability-modular-contracts-composed-proofs)).
- **Tooling obligation (backlog)**: the CI linter must enumerate every `native_decide` occurrence and verify it is either in a designated regression module or attached to an exhaustive finite-∀ proposition.
- **Proposed amendment — extend this Law's text to name `bv_decide` explicitly (PA1 gate-tooling task finding, 2026-08-27).** This Law's own text currently names only `native_decide`/`decide`; PA1 (the crc32 pathfinder) introduced this codebase's first `bv_decide` proofs, and a from-source investigation (see `TCB.md`'s `bv_decide` entry for the full empirical basis) found `bv_decide` is trust-equivalent to `native_decide`, not a stronger, kernel-checked alternative: on this toolchain (v4.33.1) it discharges a goal by running an external SAT solver to search for an LRAT certificate, then compiling and RUNNING (not kernel-replaying) a checker over that certificate via `Lean.Meta.nativeEqTrue` — the identical axiom-emission routine `native_decide` itself calls — and asserting the result as a declaration-local `<decl>._native.bv_decide.ax_*` axiom. No kernel-replay option (e.g. a `checkProofs`-style config) exists on this toolchain. Proposed text: *"`native_decide`/`decide`/`bv_decide` may discharge a verification obligation if and only if..."* (same exhaustive-finite-domain restriction, applied uniformly), on the basis that `bv_decide`'s SAT-certificate mechanism is exhaustive-finite-∀-shaped in exactly the way this Law already admits `native_decide`/`decide` for, and its axiom footprint is the same shape/trust class as theirs — not because it needs a lesser or separate rule. **Status**: proposed, not yet ratified. This is an owner-level call about the trust base (whether `bv_decide`'s SAT-solver-plus-native-eval mechanism should be accepted at all, accepted with this Law's existing restriction, or restricted further/separately) rather than one an implementation task should decide unilaterally; `scripts/gate_allowlist.txt`'s four current `bv_decide` entries are, in the meantime, categorized `finite-forall` under this Law's EXISTING text (each is an ordinary Lean theorem universally quantified, via typed parameters, over a complete finite bitvector domain — `UInt32`, or a `UInt32 × UInt8` pair — reviewed as satisfying the "exhaustive finite-∀" bullet above on the same grounds as a `native_decide`/`for..in`/`List.range` proof, not on a bv_decide-specific exception), which this proposal would make textually explicit rather than change.
