...
- **The three categories**:
  - `finite-forall` — an existing native-evaluation dependency whose proposition is universally quantified over its entire finite domain. It is still migration debt; the category records that the proposition shape was honest, not that the mechanism is permanent. The Python pre-check additionally requires a shallow syntactic corroboration signal (an explicit `∀`, a `for .. in [a:b]` range, a `List.range N>1`, or a non-literal `.all`/`.any`) between the declaration header and the occurrence — a sanity check, not a proof.
...
- **The standard-axiom set**: `{propext, Classical.choice, Quot.sound}` — the only axioms Lean's kernel trusts as part of ordinary, sorry-free, native-eval-free mathematics. A new declaration depending on any axiom outside this set (a native-evaluation axiom, `sorryAx`, or a hand-declared `axiom`) fails. Exact historical dependencies may match the shrinking temporary debt ledger only until their constructive replacements land; the ledger cannot authorize new debt.
