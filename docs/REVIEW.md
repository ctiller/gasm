...
- The repository provides two independent, automated verification tools -- see §4.1.2 for why declaration coverage was split out of `scripts/check_refs.py` into its own compiled-environment-driven tool rather than staying a second job for the same regex-based script:
  ...
  2. `python scripts/run_full_refs_coverage.py --full-repository` (`Tools/CheckRefsCoverage.lean`) detects any Lean declaration lacking a `REF:` annotation (failing CI on un-cited code, Law 1) by walking the COMPILED ENVIRONMENT rather than source text, so no declaration syntax form can hide from it. This is an explicitly opted-in full-repository gate, not an edit-local check.
...
### Law 10: Kernel-Checked Gates & Native-Evaluation Debt
> **New verification obligations must produce a kernel-checkable proof. `decide` is admissible on a complete finite domain because the kernel evaluates it. Existing `native_decide`/`bv_decide` dependencies are migration debt even when exhaustive: they may be removed but never added or substituted. A future finite-domain mechanism is admissible only after it is separately shown to produce a proof the kernel checks.**
...
2. **`decide`**. The *kernel itself* performs the evaluation as part of type-checking; no axiom is introduced, so a `decide` occurrence is accepted directly — the kernel's own check already is the gate. This rung is categorically different from rungs 3 and 4, not merely a cheaper version of the same trust: it is genuinely kernel-checked, they are not. Feasible only on domains small enough for the kernel's own evaluator to walk directly.
...
- **Exhaustive finite-∀ is an acceptable proposition shape, not a waiver for its proof mechanism**: `decide` or a structural/proof-producing solver may discharge it. Native-evaluation axioms remain existing-only debt even when the quantified domain is complete.
- **The finite domain must be the specification's actual input domain** (or an exhaustive cover of it) — exhaustiveness over a purpose-built stand-in (a single-constructor enum, `Unit`, a shrunken index type) is domain-shrinking under Law 9 and remains prohibited at every rung; Law 10 never licenses what Law 9 forbids. Any remaining spike contract of this shape is migration backlog, not a compliant instance.
...
- **Rungs 3 and 4 are gated debt; rung 2 is kernel proof**: a bare `decide` introduces no axiom, so the kernel itself checks it. Existing `native_decide`/`bv_decide` dependencies are recorded only as a shrinking migration backlog in the temporary Law-10 ledger. New entries are forbidden: a new occurrence must be replaced by a kernel/constructive proof or by a separately ratified proof-producing finite-domain mechanism before it can land.
...
**Status**: the proof-mechanism ratchet above supersedes the earlier policy that admitted new exhaustive native-evaluation entries. Rungs 3–4 describe existing debt and its trust cost, not an authoring option.
...
2. **Traceability Indexing:** `python scripts/check_refs.py` must return exit code 0, AND `python scripts/run_full_refs_coverage.py --full-repository` -- **run from the repository root, after a build; building it is not running it, the same distinction item 4 draws for `check_gates_axioms`** -- must also return exit 0. The explicit flag acknowledges that this is the authoritative full-repository compiled-environment gate: it can schedule hundreds of modules and, depending on cache state and concurrency, consume substantial time and memory. Focused module builds are appropriate for edit-local feedback but do not replace this final gate. Together these give: every `REF:` citation whose target is a `docs/` path and every explicit internal `docs/<file>.md#<anchor>` cross-link gets full anchor-existence validation against that file's real on-disk headings, zero exceptions (`check_refs.py`); every `REF:` citation whose target is a `references.json` slug is validated for slug EXISTENCE by `check_refs.py`, with byte-hash and anchor correctness delegated to item 3's scheduled `--refresh --all` or cache-primed local `--offline` audit (Law 6); and every reportable Lean declaration -- enumerated from the COMPILED ENVIRONMENT, so no declaration syntax form can be invisible to it -- has its own preceding `REF:` citation, with no exception path (`check_refs_coverage`). See §4.1.2 for why declaration coverage and citation validity are two independent tools rather than one script doing both jobs, and for the concrete history of why "100% citation validity, zero un-cited declarations" was previously an inflated claim: a regex-based single-script design could not see roughly a tenth of the codebase's own declarations (every anonymous `instance`, every `abbrev`, the one `initialize`) and silently dropped whatever citation preceded them along with it.
...
4. **Gate Policy Compliance (Law 10):** `python scripts/check_gates.py` (fast source-level
   ...
   evaluation axioms, `sorryAx`, and hand-declared axioms with no exception path. The former
...
- **Zero exceptions:** every source occurrence of `native_decide` or `bv_decide` is a hard failure.
  The compiled-environment gate likewise rejects every declaration that depends on a non-standard
  axiom. There is no ledger, parser, category, waiver, or grandfathering mechanism.
- **The standard-axiom set**: `{propext, Classical.choice, Quot.sound}` — the only axioms Lean's kernel trusts as part of ordinary, sorry-free, native-eval-free mathematics. Any declaration depending on an axiom outside this set (a native-evaluation axiom, `sorryAx`, or a hand-declared `axiom`) fails.
...
12. **x86-64 Instruction Obligation Gate:** `lake exe check_x86_obligations` — run from the
    ...
    strings, with no opt-out path. Calibration remains explicitly unvalidated until the
...
- **Unconditional declaration coverage**: every reportable compiled declaration must carry its own preceding `REF:` citation. There is no declaration-reference allowlist or exemption category. `scripts/run_gates.py --self-test` plants an uncited anonymous instance—the syntax form that exposed the original regex blind spot—and requires the compiled-environment gate to turn red before rebuilding and confirming green after removal.
