...
- **Module-based scoping**: a declaration is in scope for the axiom tool if it was *compiled from* a project module (`env.getModuleIdxFor?` names a module under the `Gasm`/`Stdlib`/`Spikes` umbrella roots) — not if its own name merely looks like it lives under one of those namespaces. The set that scoping is checked *against* — the ground truth the compiled environment is cross-referenced with, so that a module the tool cannot load is a named failure rather than a silent omission (`TCB.md` T2 / TC15) — is derived, never assumed: every **tracked** `.lean` file (`git ls-files`, never a filesystem walk) that some `lakefile.toml` build root reaches transitively, with **both** `[[lean_lib]] roots` and `[[lean_exe]] root` counting as roots. Both halves are load-bearing and both were once wrong. *Tracked*, because a filesystem walk also sees an agent's uncommitted work-in-progress: on 2026-08-28 this gate returned exit 1 and exit 0 roughly an hour apart on the same code, differing only in whether `Stdlib/Zlib/CanonicalTableSpec.lean` had been committed yet, and it reported that as an unloadable module rather than as the real cause. *Every declared target*, because modelling the build as the three `[[lean_lib]]` umbrellas alone understates what `lake build` compiles — every `Spikes/*/Emit.lean`, every `Test.lean` and every fuzzer CLI is a `[[lean_exe]]` root, compiled exactly as a library umbrella is — and leaves no way to tell those apart from a module the build compiles *nowhere*, which is a real defect but one `scripts/check_orphan_modules.py` names actionably (file, umbrella, exact `import` line) where this tool could only produce a bare exit 1 beside "0 NOT allowlisted". A module *inside* that closure which loads into neither the baseline environment nor a standalone per-module import remains a hard failure here; that is what stops an unbuilt committed module from being silently unverified.
...
11. **Instructions.lean Umbrella Completeness (B3):** `python scripts/check_instructions_umbrella.py` must return exit code 0. `Gasm/Targets/X86_64/Instructions.lean` is a hand-maintained "true umbrella": its import list is the only reason `Registry.lean`'s build-time environment audit can see an instruction family at all, because Lean's environment walk sees the current file's import graph and not every `.lean` Lake happens to compile. A new `Instructions/<Foo>.lean` declaring an `X86_64Instruction` instance but absent from that import list is therefore *invisible* to that audit rather than flagged by it — it is not reported missing, because the audit never learns it exists. The script is a direct filesystem-vs-import-list diff, checked in both directions: a family missing from the umbrella, and an import naming a file that no longer exists on disk. A file counts as a family iff it structurally declares at least one `instance : X86_64Instruction ...`, so shared infrastructure (`Base.lean`, `Obligations.lean`) never needs a hand-maintained exclusion list. `docs/tasks/B3-stage-b-decoder-modularization.md` flagged this script as residual follow-up: it existed on disk while being invoked by neither `scripts/run_gates.py`'s gate table nor `.github/workflows/ci.yml` — the identical "a gate that exists but never runs protects nothing" shape as item 5's `check_licenses.py`. That note gave the script's missing mutation test as the reason it stayed unwired, so the wiring came with one: `python scripts/check_instructions_umbrella.py --self-test` plants a real unimported family file and asserts the gate goes red naming it, reverts, and asserts green again, plus a negative control that an un-imported *non*-family file (shared infrastructure like `Base.lean`) is correctly ignored — the same planted-defect discipline `check_orphan_modules.py` and `check_doc_facade.py` carry. See the script's own module docstring for the full specification, including the blind spot it deliberately does not close.
