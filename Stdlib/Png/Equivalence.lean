...
`.get!` interacts with `.push` on an otherwise-arbitrary `ByteArray`. Lean core proves the
analogous facts for the derived `GetElem!` notation (`[i]!`, `ByteArray.getElem!_push_lt`/
`_eq` in `Init/Data/ByteArray/Lemmas.lean`) but not for `.get!` itself (a separate, directly
`@[extern]`-implemented function, not defined in terms of `[i]!`). These four lemmas bridge
that gap once; nothing here is PNG-specific, so `Stdlib/Zlib`'s own `Id.run`/`for`/`push`
loops (its byte-at-a-time DEFLATE/INFLATE accumulators) can reuse them directly instead of
re-deriving the bridge. -/
...
/-- Pushing a new byte never disturbs any earlier index -- the fact that lets an induction
    along scanline position carry its "already-reconstructed prefix" invariant across a
    `push`. -/
